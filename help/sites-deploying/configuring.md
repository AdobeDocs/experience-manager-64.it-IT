...
Il funzionamento è semplice, in quanto non è necessaria alcuna configurazione speciale, e quando necessario è possibile specificare semplicemente un indirizzo IP utilizzando il formato appropriato per il tipo di rete.
...
Dispatcher è  Adobe  caching e/o strumento di bilanciamento del carico. L’utilizzo del dispatcher consente inoltre di proteggere il server AEM dagli attacchi. Di conseguenza, per aumentare la sicurezza della tua istanza di AEM, puoi utilizzare Dispatcher insieme a un server web di classe Enterprise.
