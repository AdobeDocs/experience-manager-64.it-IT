...
* Per utilizzare l&#39;interfaccia touch come interfaccia utente predefinita, il percorso di destinazione **Target** deve puntare a:
...
**Day CQ WCM Debug** FilterQuesto è utile quando si sviluppa in quanto consente l&#39;uso di suffissi come ?debug=layout quando si accede a una pagina. Ad esempio, http://localhost:4502/cf#/content/geometrixx/en/support.html?debug=layout fornirà informazioni sul layout di potenziale interesse per lo sviluppatore.
...
* **URL per inviare** i dati, configura l’URL utilizzato per monitorare le statistiche della pagina (è vitale se una richiesta di tracciamento passa attraverso il dispatcher); ad esempio, il valore predefinito è  `http://localhost:4502/libs/wcm/stats/tracker`.
...
* **NIO per HTTP**, da utilizzare o meno NIO per HTTP. Valori predefiniti per true. Utilizzato solo se HTTP è abilitato.
