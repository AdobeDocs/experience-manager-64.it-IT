...
* **Nome logger** e **Formato del registro** per configurare la posizione e il formato della richiesta e la registrazione degli accessi (impostazione predefinita: `request.log`). Questo file di registro è essenziale per l’analisi delle prestazioni o delle funzionalità di debug correlate alla catena web.
   ...
   È associato al logger di richieste Sling di Apache.
...
Questo è abbinato al logger dati di richiesta personalizzabile Apache Sling.
...
* Utilizza l’installazione locale (ad esempio, utilizza `http://localhost:4502/system/console/jcrresolver`) per determinare quale Resource Resolver è attivo.
...
Ci sono [due interfacce](/help/sites-authoring/select-ui.md) disponibile in AEM:
...
**Filtro di debug Day CQ WCM** Questo è utile quando si sviluppa perché consente l&#39;uso di suffissi come ?debug=layout quando si accede a una pagina. Ad esempio, http://localhost:4502/cf#/content/geometrixx/en/support.html?debug=layout fornirà informazioni sul layout di potenziale interesse per lo sviluppatore.
...
* **URL per l’invio dei dati** configurare l’URL utilizzato per monitorare le statistiche della pagina (è fondamentale se una richiesta di tracciamento passa attraverso il dispatcher); ad esempio, il valore predefinito è `http://localhost:4502/libs/wcm/stats/tracker`.
