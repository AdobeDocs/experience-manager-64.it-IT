...
* Utilizzare l&#39;installazione locale (ad esempio, utilizzare `http://localhost:4502/system/console/jcrresolver`) per determinare quale Risolutore risorse è attivo.
...
Sono disponibili [due interfacce](/help/sites-authoring/select-ui.md) in AEM:
...
* **URL per l’invio di dati** per configurare l’URL utilizzato per il tracciamento delle statistiche delle pagine (è fondamentale se una richiesta di tracciamento passa attraverso il dispatcher); ad esempio, il valore predefinito è `http://localhost:4502/libs/wcm/stats/tracker`.
