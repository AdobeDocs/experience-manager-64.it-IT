...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><strong>Note</strong></td> 
   <td>Quando <code>/conf/global/settings/dam/adhocassetshare</code> è tecnicamente supportato per la ricerca (ha la precedenza prima di <code>/apps</code> tramite la ricerca CAConfig solita, ma dopo <code>/etc</code>), il modello può essere inserito in <code>/conf/global/settings/dam/adhocassetshare</code>. Tuttavia, questo non è consigliato in quanto non vi è alcuna interfaccia utente di runtime per facilitare la modifica del modello di e-mail</td> 
  </tr> 
 </tbody> 
</table>
   ...
   <td><p>Per allinearsi alla nuova struttura del repository:</p> 
    <ol> 
     <li>Copia tutti gli script personalizzati o modificati da <strong><code>/etc/dam/indesign/scripts</code></strong> a <strong><code>/apps/settings/dam/indesign/scripts</code></strong><br /> 
      <ol> 
       <li>Solo gli script nuovi o modificati come script non modificati forniti da AEM saranno disponibili tramite <strong><code>/libs/settings</code></strong> AEM 6.4</li> 
      </ol> </li> 
