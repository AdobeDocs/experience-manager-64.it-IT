...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><strong>Note</strong></td> 
   <td>Anche se <code>/conf/global/settings/dam/adhocassetshare</code> è tecnicamente supportato per la ricerca (ha la precedenza <code>/apps</code> tramite la ricerca CAConfig Sling, ma dopo <code>/etc</code>), il modello può essere inserito <code>/conf/global/settings/dam/adhocassetshare</code>. Tuttavia, questo non è consigliato in quanto non esiste alcuna interfaccia utente di runtime per facilitare la modifica del modello e-mail</td> 
  </tr> 
 </tbody> 
</table>
