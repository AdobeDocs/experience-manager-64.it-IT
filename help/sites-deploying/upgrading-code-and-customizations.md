...
1. **Sviluppare una base di codici per la versione 6.4**  - Creare un ramo o un repository dedicato per la base di codice per la versione di Target. Utilizzate le informazioni provenienti dalla compatibilità pre-aggiornamento per pianificare aree di codice da aggiornare.
1. **Compilare con 6.4 Uber jar**  - Aggiornare i POM di base del codice per puntare a 6.4 uber jar e compilare il codice in base a questo.
...
1. **Distribuisci in ambiente**  6.4 - Un&#39;istanza pulita di AEM 6.4 (Autore + Pubblicazione) dovrebbe essere presente in un ambiente Dev/QA. È necessario aggiornare la base di codice e distribuire un campione rappresentativo di contenuto (proveniente dalla produzione corrente).
1. **Convalida QA e correzione**  di bug - QA deve convalidare l&#39;applicazione sia nelle istanze Author che Publish di 6.4. Eventuali bug rilevati devono essere corretti e impegnati nella base di codici 6.4. Ripetere Dev-Cycle secondo necessità fino a risolvere tutti i bug.
...
1. Nell&#39;istanza che deve essere aggiornata, aprire il CRXDE Lite andando su `https://server:port/crx/de/index.jsp`
...
Per generare ID di risorse per le risorse esistenti, aggiornate le risorse quando aggiornate l’istanza AEM in modo da eseguire AEM 6.4. Questo è necessario per abilitare la funzione [Informazioni sulle risorse](/help/assets/touch-ui-asset-insights.md). Per ulteriori dettagli, vedere [Aggiunta di codice da incorporare](/help/assets/touch-ui-using-page-tracker.md#adding-embed-code).
