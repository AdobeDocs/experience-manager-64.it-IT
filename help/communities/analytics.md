...
Effettuando l&#39;accesso ad [Adobe Marketing Cloud](https://docs.adobe.com/content/help/en/analytics/analyze/analysis-workspace/home.html) con Nome [società e Nome utente](analytics.md#prerequisites), è possibile configurare una suite di rapporti nuova o esistente con:
...
Per un sito **AEM** generico che include componenti Community, ma non viene creato con la creazione guidata [del](sites-console.md)sito, è necessario aggiungere l&#39;elemento. Il valore di href deve essere il percorso del sito. Ad esempio, se il percorso del sito è `/content/my/company/en`, utilizzate:
...
Se la distribuzione scelta è una farm [di](topologies.md#tarmk-publish-farm)pubblicazione, un&#39;istanza di pubblicazione AEM deve essere identificata come editore principale per il polling  Adobe Analytics per i dati del report da scrivere all&#39; [SRP](working-with-srp.md).
...
L’intervallo di polling controlla la frequenza delle richieste inviate a  Adobe Analytics per estrarre e salvare i dati nell’ [SRP](working-with-srp.md).

Quando i dati possono essere classificati come &quot;big data&quot;, sondaggi più frequenti potrebbero mettere un carico elevato sul sito della comunità.
...
* Passa al [CRXDE Lite](../../help/sites-developing/developing-with-crxde-lite.md)
