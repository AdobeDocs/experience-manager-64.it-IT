...
Quando il [topologia](topologies.md) è un [pubblica azienda](../../help/sites-deploying/recommended-deploys.md#tarmk-farm), la registrazione e le modifiche apportate a un’istanza di pubblicazione devono essere sincronizzate con altre istanze di pubblicazione. I membri devono essere in grado di accedere e visualizzare i propri dati su qualsiasi nodo di pubblicazione.
...
Sull&#39;autore, quando un sito community viene pubblicato da [Console Sites di Communities](sites-console.md), l&#39;effetto è [replicare](../../help/sites-deploying/configuring.md#replication-reverse-replication-and-replication-agents) le pagine associate e Sling distribuiscono i gruppi di utenti della community creati dinamicamente, inclusa la loro appartenenza.
...
Quando la topologia è un [pubblica azienda](topologies.md) e la sincronizzazione utente è stata configurata correttamente, *user* e *profilo utente* viene sincronizzato nella farm di pubblicazione utilizzando la distribuzione Sling.
...
Per il contenuto generato dall’utente (UGC), i dati immessi in un’istanza di pubblicazione sono accessibili tramite il [SRP configurato](srp-config.md).
...
L&#39;istanza dell&#39;autore esegue il ping dell&#39;endpoint dell&#39;esportatore degli editori. Ogni volta che un utente viene creato o aggiornato su editori specifici (n), l’autore ottiene il contenuto dagli endpoint di esportazione e [invia il contenuto](sync.md#main-pars-image-1413756164) ad altri editori (n-1, a parte gli editori da cui viene recuperato il contenuto).
...
1. Accedere al [Console web](https://helpx.adobe.com/experience-manager/6-4/sites/deploying/using/configuring-osgi.html).

   Ad esempio: [http://localhost:4502/system/console/configMgr](http://localhost:4502/system/console/configMgr).
1. Individua **[!UICONTROL Agente di distribuzione Apache Sling - Sync Agent Factory]**.

   * Seleziona la configurazione esistente da aprire per la modifica (icona a forma di matita).
   * Nome verifica: **`socialpubsync`.**
   * Seleziona la casella di controllo **[!UICONTROL Abilitato]**.
   * Seleziona **[!UICONTROL Utilizzare più code]**.
   * Specifica **[!UICONTROL Endpoint per l’esportazione]** e **[!UICONTROL Endpoint importazione]** (puoi aggiungere altri endpoint di esportazione e importazione).
...
1. Seleziona la configurazione esistente da aprire per la modifica (icona a forma di matita).
   ...
   Verifica, proprietà `name:` **`socialpubsync`\- `publishUser` .**
...
1. Individua **[!UICONTROL Agente di distribuzione Apache Sling - Queue Agent Factory]**.
1. Seleziona la configurazione esistente da aprire per la modifica (icona a forma di matita).

   Nome verifica: `socialpubsync` \-inversa.
1. Seleziona la **[!UICONTROL Abilitato]** e salva.
1. Specifica i percorsi dei nodi in cui replicare **[!UICONTROL Radici ammesse]**.
1. Ripeti per ogni `publish` istanza.
...
1. Seleziona la configurazione esistente da aprire per la modifica (icona a forma di matita).
   ...
   Verifica **[!UICONTROL nome dell&#39;agente]**: `socialpubsync` \&amp;reverse;&amp;ast;.
1. Seleziona la casella di controllo **[!UICONTROL Abilitato]**.
1. Specifica **rep`:members`** come `description` per propertyName in **[!UICONTROL nomi delle proprietà di ricerca]** e Salva.
...
1. Accedi con privilegi di amministratore.
1. Accedere al [Console web](../../help/sites-deploying/configuring-osgi.md)ad esempio, [http://localhost:4502/system/console/configMgr](http://localhost:4502/system/console/configMgr)
1. Individua **[!UICONTROL Trigger di distribuzione Apache Sling - Scheduled Triggers Factory]**

   * Seleziona la configurazione esistente da aprire per la modifica (icona a forma di matita)
   * Verifica `Name:` **`socialpubsync`trigger programmato**
...
Per i problemi nella distribuzione Sling in cui vi è una discrepanza nelle sottoscrizioni e seguenti, controlla se le seguenti proprietà in **[!UICONTROL Listener di sincronizzazione utenti di AEM Communities]** le configurazioni sono impostate:
...
1. Accedi con privilegi di amministratore.
1. Accedere al [Console web](../../help/sites-deploying/configuring-osgi.md). Ad esempio: [http://localhost:4503/system/console/configMgr](http://localhost:4503/system/console/configMgr).
1. Individua **[!UICONTROL Listener di sincronizzazione utenti di AEM Communities]**.
1. Seleziona la configurazione esistente da aprire per la modifica (icona a forma di matita).

   Nome verifica: **`socialpubsync`trigger programmato**
...
1. Aggiungi tutte le cartelle in cui eseguire la sincronizzazione **[!UICONTROL Cartelle distribuite]**. Ad esempio:
...
1. Sfoglia per [https://_host:porta_/system/console/status-slingsettings](http://localhost:4503/system/console/status-slingsettings).
...
1. In `crx-quickstart/launchpad/felix` , cerca ed elimina il file denominato _sling.id.file.
...
1. Convalidare che **[!UICONTROL Sling ID]** è ora univoco.
...
1. Accedere al [Console web](../../help/sites-deploying/configuring-osgi.md)ad esempio, [http://localhost:4503/system/console/configMgr](http://localhost:4503/system/console/configMgr).
1. Individua il **[!UICONTROL Pacchetto di distribuzione Apache Sling - Vault Package Builder Nome di fabbrica]**: socialpubsync-vlt.
...
1. Gestione dei criteri
   * Per sovrascrivere il rep esistente `:policy` nodi con nuovi, aggiungi un terzo filtro pacchetti:
...
1. **Controllare le configurazioni**. Assicurati che tutte le [configurazioni](sync.md#bestpractices) sono impostati in modo appropriato nell’istanza di AEM Author, come indicato in [Best practice](sync.md#main-pars-header-863110628).
1. **Verifica autorizzazioni utente autorizzate**. Se i pacchetti non sono installati correttamente, controlla che il [utente autorizzato](../../help/sites-administering/sync.md#createauthuser) creato nella prima istanza Publish ha le ACL corrette.
...
1. **Controlla i registri generati durante la creazione dell’utente nell’istanza Publish**. Se le configurazioni di cui sopra sono impostate in modo appropriato ma la sincronizzazione utente non funziona, controlla i registri generati al momento della creazione dell’utente.
   ...
   1. Su AEM’istanza di authoring, accedi con privilegi di amministratore.
      ...
      1. Accedere al [Console web](../../help/sites-deploying/configuring-osgi.md). Ad esempio: [http://localhost:4502/system/console/configMgr](http://localhost:4502/system/console/configMgr).
      1. Individua la configurazione **[!UICONTROL Agente di distribuzione Apache Sling - Sync Agent Factory]**.

      1. Deseleziona la **[!UICONTROL Abilitato]** casella di controllo.
      Quando si disabilita la sincronizzazione utente nell’istanza di authoring, gli endpoint (esportazione e importazione) sono disabilitati e l’istanza di authoring è statica. La **[!UICONTROL vlt]** i pacchetti non vengono ping o recuperati dall’autore.

      Ora, se un utente viene creato su un&#39;istanza di pubblicazione, il **[!UICONTROL vlt]** il pacchetto viene creato in */var/sling/distribution/packages/ socialpubsync - vlt /data* nodo. E se questi pacchetti vengono inviati dall&#39;autore a un altro servizio. Puoi scaricare ed estrarre questi dati per controllare quali proprietà vengono inviate ad altri servizi.

   1. Passa a un editore e crea un utente sull&#39;editore. Di conseguenza, vengono creati degli eventi.
   1. Controlla la [ordine dei registri](sync.md#troubleshoot-sling-distribution-in-aem-communities), creato al momento della creazione dell’utente.
   1. Controlla se un **[!UICONTROL vlt]** il pacchetto viene creato il `/var/sling/distribution/packages/socialpubsync-vlt/data`.
   1. Ora, abilita la sincronizzazione utente sull&#39;istanza AEM autore.
   1. Su editore, modifica gli endpoint di esportazione o importazione in **[!UICONTROL Agente di distribuzione Apache Sling - Sync Agent Factory]**.
