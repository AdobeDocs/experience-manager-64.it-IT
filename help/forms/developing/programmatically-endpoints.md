...
* **filePattern**: Specifica i pattern di file allegato in ingresso accettati dal provider. Ciò include i file con estensioni di nome file specifiche (&amp;ast;.dat, &amp;ast;.xml), i file con nomi specifici (dati) e i file con espressioni composite nel nome e nell&#39;estensione (&amp;ast;.[dD][aA][Tt]). Il valore predefinito è `*`.
