...
1. Archiviare il modulo PDF in Content Services (obsoleto) utilizzando il servizio Document Management
   ...
   * Un `java.util.Map` oggetto che memorizza gli attributi del contenuto.
   ...
   Il `storeContent` metodo restituisce un `CRCResult` oggetto che descrive il contenuto. Utilizzando un `CRCResult` oggetto potete, ad esempio, ottenere il valore di identificatore univoco del contenuto. Per eseguire questa operazione, richiamare il `CRCResult` metodo dell&#39; `getNodeUuid` .
