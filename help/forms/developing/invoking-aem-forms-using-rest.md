...
Per richiamare un servizio Forms (un processo diventa un servizio quando il processo viene attivato) utilizzando REST, configurate un endpoint REST. (vedere &quot;Gestione degli endpoint&quot; nella guida [](https://www.adobe.com/go/learn_aemforms_admin_63)di amministrazione.)
...
* Oggetti della raccolta quali `java.util.List` e `java.util.Map`
   ...
   Se un servizio Forms richiede un `com.adobe.idp.Document` parametro, il servizio può essere invocato solo utilizzando il metodo POST HTTP. Se il servizio richiede un `com.adobe.idp.Document` parametro, il corpo della richiesta HTTP diventa il contenuto dell&#39;oggetto Document di input.
...
Alcuni servizi AEM Forms , come i processi a lunga durata incentrati sull’uomo, richiedono molto tempo per essere completati. Questi servizi possono essere richiamati in modo asincrono in modo non bloccante. (Vedete [Richiamo Di Processi](/help/forms/developing/invoking-human-centric-long-lived.md#invoking-human-centric-long-lived-processes)Lunghi Orientati All’Umano.)
