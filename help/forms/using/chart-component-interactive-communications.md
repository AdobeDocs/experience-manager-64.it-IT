...
1. Toccate il componente grafico nell&#39;editor di comunicazione interattiva e, dalla barra degli strumenti del componente, selezionate **[!UICONTROL Configura (]** ![configura_icon](assets/configure_icon.png)).
...
1. Configurare le proprietà di base del grafico per il canale di stampa e il canale Web. A parte le proprietà comuni, esistono proprietà specifiche per la stampa e il canale Web e il tipo di grafico.
   ...
   * Specificate quanto segue per l&#39;asse **[!UICONTROL x]** e l&#39;asse y **[!UICONTROL y]**:
      ...
      * **[!UICONTROL Funzione]**: Per utilizzare le funzioni statistiche per calcolare i valori sull&#39;asse, selezionare la funzione per l&#39;asse X/Y. Per ulteriori informazioni sulle funzioni, vedere [Utilizzare le funzioni nel grafico](#usefunction) e [Esempio 2: Applicazione delle funzioni somma e media in un grafico a linee](#applicationsumfrequency).
...
1. (Solo canale di stampa) Nelle Impostazioni agente, specificare se l&#39;agente deve utilizzare questo grafico. Se l&#39;opzione i **[!UICONTROL t è obbligatoria perché l&#39;agente utilizzi questo grafico]** non è selezionata, l&#39;agente può toccare l&#39;icona occhio per il grafico nella scheda Contenuto dell&#39;interfaccia utente dell&#39;agente per mostrare o nascondere il grafico.

   ![chart_agentproperties](assets/chart_agentproperties.png)

1. Nella barra laterale Proprietà, toccare ![done_icon](assets/done_icon.png).
...
* **[!UICONTROL Titolo]** per gli assi X e Y (per il tipo di colonna grafico nel canale di stampa solo in questo esempio) - in questo esempio, Importo ($) per l&#39;asse X e Spese per l&#39;asse Y.
...
* **[!UICONTROL Descrizione]** per visualizzare una descrizione al passaggio del mouse su una spesa (solo per il canale Web) - in questo esempio  `${x}: $ ${y}`, che viene visualizzata come  `[Expense Label: $ Amount]` (Esempio: Visita del parco tematico: $ 315)

![Grafico a colonne nell’output di stampa di un grafico di ](assets/chartprintchannel.png)
**comunicazione interattivaFigura:grafico a** *colonne nell’output di stampa di una comunicazione interattiva*

**Asse A.** Y - Importo recuperato dalla proprietà del modello dati del modulo e dalla proprietà Titolo impostata su Importo ($)  **B.** Label Direction of X axis impostato su Inclinazione a sinistra su Asse  **C.** X - Descrizione delle spese recuperata dalla proprietà del modello dati del modulo e proprietà Titolo impostata su Spese

![Grafico ad anello nell&#39;output Web di un grafico di ](assets/chartwebchannel.png)
**comunicazione interattivaFigura:** *Ciambella nell&#39;output web di una comunicazione interattiva*

**A.** La proprietà Raggio interno della ciambella è impostata su  **B.** Mostra la proprietà Legenda è selezionata e la proprietà Posizione legenda è impostata su  **Destra.** Descrizione comandi visualizza il dettaglio dell&#39;elemento al passaggio del mouse. La descrizione comandi è impostata su ${x}: $ ${y}
...
**** Nome: identificatore per l’elemento grafico. Il nome non è visibile sul grafico, ma è utile per fare riferimento all&#39;elemento di altri componenti, script ed espressioni SOM.

**Titolo (solo canale di stampa)** Specifica il titolo del grafico.

**Tipo** di grafico: consente di specificare il tipo di grafico da generare. Le opzioni disponibili sono Torta, Colonna, Anello, Barra (solo canale Web), Linea, Linea e Punto, Punto e Area. Per ulteriori informazioni, vedere l&#39;esempio 1: Output grafico in stampa e Web.
...
**Asse X >** Direzione etichettaDirezione dell’etichetta sul grafico nel canale di stampa. Se si sceglie la direzione dell&#39;etichetta come Rotazione personalizzata, viene visualizzato il campo Angolo di rotazione personalizzato (gradi). Nel campo Angolo di rotazione personalizzato (gradi), potete scegliere l’angolo di rotazione in passaggi di 15 gradi.

**Asse Y >** Titolo: specifica il titolo dell’asse y.

**Asse Y > Oggetto modello dati &amp;ast;** Specifica l&#39;elemento della raccolta dati del modulo da tracciare sull&#39;asse y. Nel canale Stampa, l&#39;oggetto del modello dati per l&#39;asse Y deve essere di tipo Numero.
...
**Mostra** legendaMostra una legenda per il grafico a torta o a torta quando è attivato.
...
**Descrizione (solo per il canale Web)** Specifica il formato in cui la descrizione comando viene visualizzata al passaggio del mouse su un punto dati del grafico nel canale Web. Il valore predefinito è \${x}(\${y}). A seconda del tipo di grafico, quando si posiziona il mouse su un punto, una barra o una sezione del grafico, le variabili \${x} e \${y} vengono sostituite dinamicamente con i valori corrispondenti sull&#39;asse x e sull&#39;asse y e visualizzate nella descrizione comandi.
...
**Rientro (solo canale di stampa)** Specificate il rientro del grafico a sinistra della pagina.

**Configurazioni specifiche per i graficiOltre alle configurazioni comuni, sono disponibili le seguenti configurazioni specifiche per i grafici:** 
...
## Utilizzare le funzioni nel grafico {#usefunction}
...
**** ModalitàRestituisce il valore con la maggior parte delle occorrenze sull&#39;asse X o Y per un valore specificato sull&#39;altro asse
