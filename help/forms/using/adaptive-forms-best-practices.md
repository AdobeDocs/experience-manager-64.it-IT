...
* Utilizzare `this` l&#39;operatore per un componente per fare riferimento a se stesso in un&#39;espressione di regola. La regola rimane valida anche se il nome del componente cambia. Ad esempio, `field1.valueCommit script: this.value > 10`.

* Utilizzare i nomi dei componenti per fare riferimento ad altri componenti del modulo. Utilizzare la `value` proprietà per recuperare il valore di un campo o di un componente. Ad esempio, `field1.value`.

* Per evitare conflitti, fai riferimento ai componenti in base a una gerarchia univoca relativa. Ad esempio, `parentName.fieldName`.
