...
>[!NOTE]
>
>È possibile modificare il tema colore per le righe di intestazione e le righe corpo modificando i valori delle variabili LESS. Per ulteriori informazioni, vedere [Temi in  AEM Forms](/help/forms/using/themes.md).
...
In fase di esecuzione, verranno visualizzati **`+`** e *`-`* pulsanti per aggiungere o eliminare una riga.
...
Per unire questi dati alla tabella in fase di esecuzione, è necessario indirizzare il modulo adattivo contenente la tabella nella posizione XML assoluta con wcmmode disattivato. Ad esempio, se il modulo adattivo si trova in *http://localhost:4502/myForms/bankTransaction.html* e il file XML di dati viene salvato in *C:/myTransactions/bankSummary.xml*, è possibile visualizzare la tabella con i dati al seguente URL:
