...
**Testo di benvenuto:** Testo del messaggio di benvenuto, ad esempio &quot;Accedi con il tuo nome utente e la password&quot;. The welcome message text should contain information on how to log in to document security and how to contact an administrator or other designated support person in your organization for assistance. Ad esempio, gli utenti esterni potrebbero dover contattare un amministratore se dimenticano la password o necessitano di assistenza per il processo di registrazione o di accesso. La lunghezza massima del testo di benvenuto è di 512 caratteri.
...
Le filigrane non sono modificabili e pertanto rappresentano un metodo più sicuro per garantire la riservatezza del contenuto del documento. Dynamic watermarks also ensure that a watermark shows enough user-specific information to act as a deterrent to further distributing the document.

La filigrana specificata da un criterio viene visualizzata nel documento protetto tramite criterio quando il destinatario visualizza o stampa il documento. Unlike permanent watermarks, a dynamic watermark is never saved in the document, which provides the flexibility that is necessary when deploying a document in an intranet environment to ensure that the viewing application displays the identity of the specific user. Inoltre, se un documento ha più utenti, l&#39;uso della filigrana dinamica consente di utilizzare un documento invece di più versioni, ognuna con una filigrana diversa. La filigrana visualizzata riflette l&#39;identità dell&#39;utente corrente.
...
1. Selezionare Adatta alla pagina oppure selezionare % e digitare una percentuale nella casella. Il valore deve essere un numero intero, non una frazione. To configure the watermark size, you can use a value that is the percentage of the page or set the watermark to fit the size of the page.
1. In the Rotation box, type the degrees by which to rotate the watermark. The range is from -180 to 180. Use a negative value to rotate the watermark counterclockwise. Il valore deve essere un numero intero, non una frazione.
...
Users who are external to your organization can register with document security. Invited users who register and activate their accounts can log in to document security by using their email address and the password they create when they register. Gli utenti invitati registrati possono utilizzare documenti protetti tramite criterio per i quali dispongono delle autorizzazioni.

Quando gli utenti invitati vengono attivati, diventano utenti locali. Gli utenti locali possono essere configurati e gestiti utilizzando l&#39;area Utenti invitati e locali. (See [Managing invited and local user accounts](/help/forms/using/admin-help/invited-local-user-accounts.md#managing-invited-and-local-user-accounts).)
...
**Email Encoding:** Encoding format to use when sending email messages. È possibile specificare qualsiasi codifica, ad esempio UTF-8 per Unicode o ISO-8859-1 per Latin. The default is UTF-8.

**Redirect Email Address:** When you specify an email address for this setting, any new invitation are sent to the address provided. Questa impostazione può essere utile a scopo di test.
...
Document security automatically issues a registration invitation email when you create a new invited user account or when an existing user adds an external recipient who has not previously registered or been invited to register to a policy. Il messaggio e-mail contiene un collegamento che il destinatario può utilizzare per accedere alla pagina di registrazione e immettere le informazioni relative all&#39;account personale, inclusi nome utente e password. The password can be any combination of eight characters.
...
**Timeout:** The number of days after which the registration invitation expires if the external user does not register. Il valore predefinito è 30 giorni.
...
Dopo la registrazione degli utenti invitati, Document Security invia un messaggio e-mail di attivazione. The activation email contains a link to the account activation page where the users can activate their account. Quando gli account sono attivati, gli utenti possono accedere alla protezione dei documenti utilizzando il proprio indirizzo e-mail e la password che hanno creato al momento della registrazione.
