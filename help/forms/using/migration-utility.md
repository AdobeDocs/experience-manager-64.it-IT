...
L’utility Migrazione converte le risorse [Moduli](/help/forms/using/introduction-forms-authoring.md)adattivi, le configurazioni [](/help/sites-developing/extending-cloud-config.md)cloud e le risorse [Gestione](/help/forms/using/cm-overview.md) corrispondenza dal formato utilizzato nelle versioni precedenti al formato utilizzato in AEM 6.4 Forms. Quando si esegue l&#39;utility di migrazione, vengono migrati i seguenti elementi:
...
Se si tratta di un&#39;installazione obsoleta (fresca), prima di poter utilizzare le risorse e i documenti, dovrete installare il pacchetto [Compatibilità](https://www.adobeaemcloud.com/content/marketplace/marketplaceProxy.html?packagePath=/content/companies/public/adobe/packages/cq640/fd/AEM-FORMS-6.4-COMPAT) AEMFD (compreso il pacchetto Compatibilità di gestione della corrispondenza).

Quindi devi importare il pacchetto di risorse (zip o cmp) nella nuova configurazione e quindi aggiornare le risorse e i documenti [eseguendo l’utility](#runningmigrationutility)Migrazione. A causa di [precedenti modifiche relative](/help/sites-deploying/backward-compatibility.md) alla compatibilità, vengono modificati i percorsi di alcune cartelle nell’archivio crx. Esportate e importate manualmente le dipendenze (librerie e risorse personalizzate) dalla configurazione precedente all’ambiente nuovo.
...
1. Per eseguire la migrazione, effettuate le seguenti operazioni:
   ...
   * Regole e script creati con l&#39;editor di regole (6.1 FP1 e versioni successive)
   ...
   >* Per migrare i modelli (non richiesti se si esegue l&#39;aggiornamento dalla versione 6.3), toccare Migrazione modelli moduli adattivi e nella schermata successiva toccare Avvia migrazione. Viene eseguita la migrazione dei seguenti elementi:
      >
      >  
   ...
   * **Sposta layout e frammenti di layout nell&#39;interfaccia** utente di AEM 6.4 Forms: Se si sta effettuando l&#39;aggiornamento dalla versione 6.2 alla 6.4, i modelli di layout e i frammenti di layout vengono aggiunti come moduli nella sezione dell&#39;interfaccia utente di AEM Forms 6.4.
   >[!NOTE]
   >
   >Se state effettuando l’aggiornamento dalla versione 6.2 alla 6.4, per Gestione corrispondenza potrebbero essere visualizzate nuove cartelle nell’interfaccia utente che includono le risorse. Per individuare le risorse, potrebbe essere necessario controllare queste cartelle.
...
Dopo aver eseguito l&#39;utility Migrazione, effettua le seguenti operazioni di pulizia:
...
1. Verificate che la versione XFA di layout e layout di frammenti sia 3.3 o successiva. Se si utilizzano layout e layout di frammenti di una versione precedente, potrebbero verificarsi problemi durante il rendering della lettera. Per aggiornare la versione di un XFA precedente all&#39;ultima versione, effettua i seguenti passaggi:
   ...
   1. [Scaricare l&#39;XFA come file](/help/forms/using/import-export-forms-templates.md#p-import-and-export-assets-in-correspondence-management-p) ZIP dall&#39;interfaccia utente di Forms.
   1. Estrarre il file.
