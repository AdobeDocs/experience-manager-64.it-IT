...
* File con estensioni di nomi di file specifiche; ad esempio, &amp;ast;.dat, &amp;ast;.xml, .pdf, &amp;ast;.&amp;ast;
...
1. [Creare uno script ECMAScript](/help/forms/using/watched-folder-in-aem-forms.md#p-create-an-ecmascript-p)
1. [Creare un flusso di lavoro](/help/forms/using/watched-folder-in-aem-forms.md#p-create-a-workflow-p)
1. [Configurare la cartella controllata](/help/forms/using/watched-folder-in-aem-forms.md#p-configure-the-watched-folder-p)
...
1. Apri AEM’interfaccia utente del flusso di lavoro in una finestra del browser.
   ...
   https://[nome server]:[porta]/workflow
...
1. Apri CRXDE lite in una finestra del browser. `https://[server]:[port]/crx/de/`
...
È possibile configurare una cartella sottoposta a controllo per avviare un flusso di lavoro, un servizio o uno script per elaborare i file di input. Nella sezione seguente verrà configurata una cartella sottoposta a controllo per avviare uno script ECMAScript. Il codice ECMAScript utilizzerebbe la funzionalità OutputCentralService e sendToPrinter del servizio SendToPrinterService.
...
1. Passa a /etc/workflow/scripts e crea una cartella denominata **CMB**.

1. Nella cartella CMB, crea un file denominato cmb-mergeandprint-sample.ecma e aggiungi il seguente codice al file:
...
1. Passa al nodo jcr:content del file cmb-mergeandprint-sample.ecma e aggiungi una proprietà jcr:title al nodo.

   >[!NOTE]
   >
   >Durante la creazione del flusso di lavoro, la proprietà title viene utilizzata per individuare ECMAScript.

### Creare un flusso di lavoro {#create-a-workflow-1}

1. Apri AEM’interfaccia utente del flusso di lavoro in una finestra del browser. `https://[server]:[port]/workflow`
...
1. Passa alla cartella /etc/fd/watchfolder/config/ e crea un nodo denominato CMBWatchedFolder di tipo `nt:unstructured`.
