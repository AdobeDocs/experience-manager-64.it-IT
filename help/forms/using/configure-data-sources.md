...
1. Nella finestra di dialogo di configurazione, specificate i dettagli per il database da configurare, ad esempio:
   ...
   * Nome utente e password per stabilire la connessione con il driver JDBC
   ...
   >1. Nel campo Testo **** normale, specificare la password o qualsiasi stringa da cifrare e fare clic su **[!UICONTROL Protect]**.
...
1. Vai a **[!UICONTROL Strumenti > Cloud Services > Origini]** dati. Toccate per selezionare la cartella in cui desiderate creare una configurazione cloud.
   ...
   Consultate [Configurare la cartella per le configurazioni](/help/forms/using/configure-data-sources.md#cloud-folder) del servizio cloud per informazioni sulla creazione e la configurazione di una cartella per le configurazioni del servizio cloud.
