...
seo-description: La tecnologia di imaging intelligente sfrutta le caratteristiche di visualizzazione esclusive di ogni utente per distribuire automaticamente le immagini giuste ottimizzate per la propria esperienza, migliorando le prestazioni e il coinvolgimento.
...
* Richieste PNG per ottenere immagini senza perdita di dati.
* Richieste JPEG con diverse impostazioni di qualità.
* Richieste JPEGXR a una serie di impostazioni di qualità.
* Richieste JPEG2000 con una serie di impostazioni di qualità.
* Richieste WEBP a una serie di impostazioni di qualità.
...
La funzione di imaging avanzato si basa sui predefiniti per immagini esistenti e osserva praticamente tutte le impostazioni delle immagini, come dimensione, qualità, nitidezza e così via. Ciò che cambierà sarà il formato dell&#39;immagine e le impostazioni di qualità come da richiesta. Per la conversione del formato, manteniamo la fedeltà visiva completa, come definito dalle impostazioni del predefinito per immagini, ma con file di dimensioni inferiori.
...
* Utilizzate un dominio dedicato (ovvero, `images.company.com` o `mycompany.scene7.com`), non un dominio generico (ovvero, `s7d1.scene7.com`, `s7d2.scene7.com`o `s7d13.scene7.com`).
   ...
   Tap **[!UICONTROL Setup > Application Setup > General Settings]**. Cercate il campo con l’etichetta Nome **[!UICONTROL server]** pubblicato. Se utilizzi un dominio generico, puoi richiedere di passare al dominio personalizzato come parte di questa transizione.
...
1. Facoltativo: puoi provare lo smart imaging da Staging, prima che Adobe introduca la nuova funzione in produzione.
