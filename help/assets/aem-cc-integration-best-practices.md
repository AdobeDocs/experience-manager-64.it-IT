...
description: Procedure ottimali per l’integrazione di una distribuzione AEM con Adobe Creative Cloud per semplificare i flussi di lavoro di trasferimento delle risorse e ottenere la massima efficienza
contentOwner: AG
...
| Condivisione di un numero maggiore (2) di risorse DAM con un utente Creative | ✔✔ | ✘ | [Brand Portal](https://docs.adobe.com/content/help/en/experience-manager-brand-portal/using/home.html) <br> [Condivisione risorse](assets-finder-editor.md) |
| Condivisione di risorse DAM con utenti che hanno accesso a DAM | ✔✔ | ✔ | [Condivisione collegamenti](link-sharing.md) |
| Condivisione di risorse DAM con utenti che non hanno accesso a DAM | ✘ | ✔✔ | [Brand Portal](https://docs.adobe.com/content/help/en/experience-manager-brand-portal/using/home.html) <br> [Condivisione risorse](assets-finder-editor.md) |
...
* ✘: non deve essere utilizzato per il caso di utilizzo
...
* (3) Utilizzare la funzione di caricamento delle cartelle per l&#39;app desktop AEM
...
In genere, i team creativi interni o le agenzie/i professionisti creativi caricati sulla rete interna hanno accesso all’istanza DAM, incluso l’accesso AEM.

In questi casi, l’app desktop AEM consente di accedere facilmente alle risorse finali/approvate e di salvare le risorse pronte per la creazione in DAM.
...
In questi casi, potete sfruttare l&#39;integrazione AEM/Creative Cloud per migliorare il flusso di lavoro. Il prerequisito è che gli utenti creativi dispongano di un Adobe ID e di un account Creative Cloud con servizio di archiviazione.

Utilizzate le seguenti strategie per fornire l&#39;accesso alle risorse finali/approvate:

* Per consentire l’accesso a un gran numero di risorse: Utilizzare [AEM Assets Brand Portal](https://helpx.adobe.com/experience-manager/brand-portal/user-guide.html)o l&#39;implementazione di [Asset Share](assets-finder-editor.md) da parte del cliente sull&#39;infrastruttura di pubblicazione AEM

* Per consentire l’accesso ad alcune risorse: La condivisione di cartelle AEM con Adobe Creative Cloud può essere utilizzata in aggiunta al Portale marchio di AEM Assets o a Condivisione risorse. Nota: esistono alcune limitazioni relative a questa integrazione, descritte più dettagliatamente in questo articolo.
...
1. Utilizzate (inserite) le risorse approvate in una nuova progettazione: L&#39;utente creativo produce una nuova risorsa utilizzando le risorse approvate esistenti nelle app Creative Cloud
...
* Utilizza un&#39;area/sistema di memorizzazione dedicata, ad esempio la cartella sincronizzata Risorse Adobe Creative Cloud, per i file WIP: Gli aggiornamenti frequenti non rilevanti per gli utenti DAM sono gestiti al meglio da un sistema dedicato e non da Risorse AEM. Le risorse WIP possono essere sincronizzate su disco locale tramite l’applicazione desktop Adobe Creative Cloud, salvate nell’archivio locale e così via.
...
* Richiesta di modifiche alle risorse dalla revisione e approvazione in AEM Assets
...
In tali casi, l&#39;app desktop AEM fornisce il modo più semplice per eseguire queste operazioni.
...
* Evitate di salvare direttamente un file aperto da una condivisione di rete mappata dall’app desktop AEM, a meno che le modifiche apportate al file non siano di dimensioni ridotte.
...
Se utilizzate Condivisione file di rete per gestire le risorse digitali, l&#39;utilizzo della condivisione di rete mappata dall&#39;app desktop AEM potrebbe essere considerato un comodo sostituto. Quando si passa da condivisioni di file di rete, l’interfaccia utente Web di AEM fornisce un set completo di funzionalità di gestione delle risorse digitali che vanno ben oltre quanto è possibile su una condivisione di rete (ricerca, raccolte, metadati, collaborazione, anteprime, ecc.) e l’app desktop AEM fornisce un semplice collegamento per connettere l’archivio DAM lato server al lavoro sul desktop.
