...
|  | Opzioni di PDF | È possibile rasterizzare i file, estrarre parole di ricerca e collegamenti, generare automaticamente un eCatalog, impostare la risoluzione e scegliere uno spazio colore.<br> Gli eCatalog non sono supportati in AEM. <br> Vedi [Impostazione delle opzioni di caricamento di PDF ](#setting-pdf-upload-options)<br>**Nota**: Il numero massimo di pagine per un PDF da considerare per l’estrazione è 5000 per i nuovi caricamenti. Questo limite sarà portato a 100 pagine il 31 dicembre 2022. Vedi anche [Limiti Dynamic Media](/help/assets/limitations.md). |
