...
1. Assicurati che tutte le librerie richieste siano estratte, controllando la cartella utilizzando il comando `ls -la /aem64/author/crx-quickstart/launchpad/felix/bundle<id>/data/binaries/`, dove il nome della cartella viene costruito utilizzando l&#39;ID bundle. Ad esempio, il comando è `ls -la /aem64/author/crx-quickstart/launchpad/felix/bundle588/data/binaries/` se l&#39;id del bundle è `588`.
...
1. Aggiungi il percorso `/aem64/author/crx-quickstart/launchpad/felix/bundle<id>/data/binaries/` al file conf utilizzando il comando `cat SWitchEngineLibs.conf` .

1. Esegui il comando `ldconfig` per creare i collegamenti e la cache necessari.

1. Nell’account utilizzato per avviare AEM, modifica il file `.bash_profile` . Aggiungi `LD_LIBRARY_PATH` aggiungendo quanto segue.
...
Verifica la configurazione, carica un&#39;immagine TIFF e monitora il file error.log. Noterai i messaggi `INFO` con menzioni di `SwitchEngineHandlingProcess execute: executing command line`. I registri menzionano le rappresentazioni generate. Al termine del flusso di lavoro, puoi visualizzare le nuove rappresentazioni in AEM.
