...
>
>* Consultate [Configurazione di Cloud Services per contenuti multimediali dinamici in Configurazione di elementi multimediali dinamici - Modalità ibrida.](/help/assets/config-dynamic.md)
...
1. **Per gestire i video** Contenuti multimediali dinamici, effettuate una delle seguenti operazioni:
   ...
   * Organizzare, sfogliare ed effettuare ricerche nelle risorse video
      ...
      * [Organizzazione delle risorse digitali](organize-assets.md)
         ...
         Ulteriori informazioni sulle [procedure ottimali per l&#39;organizzazione delle risorse digitali per l&#39;utilizzo dei profili di elaborazione](organize-assets.md#organize-using-folders)

      * [Ricerca di risorse](search-video-assets.md) video o [Ricerca di risorse](managing-assets-touch-ui.md#searching-assets)
...
Lo streaming video adattivo è supportato su diverse piattaforme iOS. Consultate la Guida di riferimento ai visualizzatori di [Adobi](https://docs.adobe.com/content/help/en/dynamic-media-developer-resources/library/home.html).
...
Consultate anche [I visualizzatori](https://docs.adobe.com/content/help/en/dynamic-media-developer-resources/library/viewers-for-aem-assets-only/c-html5-aem-asset-viewers.html) HTML5 nella guida di riferimento dei visualizzatori di Adobi .
...
* **Codifica** a bitrate costante (CBR) - Durante la codifica CBR, il bitrate o il numero di bit al secondo viene mantenuto invariato durante tutto il processo di codifica. La codifica CBR persiste quando la velocità dati impostata viene impostata sull’intero video. Inoltre, la codifica CBR non ottimizza i file multimediali per la qualità, ma consente di risparmiare spazio di archiviazione.

   Usate CBR se il video contiene un livello di movimento simile per l’intero video. CBR è utilizzato soprattutto per lo streaming di contenuti video. Consultate anche [Utilizzo di parametri](video-profiles.md#using-custom-added-video-encoding-parameters)di codifica video personalizzati.
...
Per pubblicare contenuti su YouTube, è necessario un account Google. Se avete un account GMAIL allora avete già un account Google. Se non avete un account Google, potete facilmente crearne uno. L’account è necessario perché sono necessarie le credenziali per pubblicare le risorse video su YouTube. Se avete già creato un account, saltate questa attività e passate a [Creazione di un canale](#creating-a-youtube-channel)YouTube.
...
1. Nell&#39;intestazione **[!UICONTROL Create un ID]** client OAuth 2.0, immettete un nome univoco.
1. Nel campo di testo sotto l’intestazione Origini **[!UICONTROL Javascript]** Autorizzate, immettete il percorso seguente, sostituendo il vostro dominio e numero di porta nel percorso, quindi premete **[!UICONTROL Invio]** per aggiungere il percorso all’elenco:
...
1. Nel campo di testo sotto l’intestazione URI **[!UICONTROL di reindirizzamento]** Autorizzati, immettete quanto segue, sostituendo il vostro dominio e numero di porta nel percorso, quindi premete Invio per aggiungere il percorso all’elenco:
...
1. Nel campo di testo sotto l’intestazione Nome **[!UICONTROL prodotto visualizzato dagli utenti]** , immettete ciò che desiderate visualizzare nella schermata di consenso.
...
1. Nell’angolo superiore destro della pagina YouTube, toccate l’immagine del profilo (potrebbe anche essere visualizzata come una lettera all’interno di un cerchio colorato in tinta unita), quindi toccate le impostazioni **** YouTube (icona a forma di ingranaggio circolare).
1. Nella pagina **[!UICONTROL Panoramica]** , sotto l’intestazione Funzioni **** aggiuntive, toccate **[!UICONTROL Visualizza tutti i canali o crea un nuovo canale]**.
1. Nella pagina **[!UICONTROL Canali]** , toccate **[!UICONTROL Crea un nuovo canale]**.
1. Nella pagina **[!UICONTROL Account]** marchio, nel campo Nome **[!UICONTROL account]** marchio, immettete un nome commerciale o un altro nome di canale per scegliere dove pubblicare le risorse video, quindi toccate **[!UICONTROL Crea]**.
...
1. Nella pagina **[!UICONTROL Agenti autore]** , toccate **[!UICONTROL YouTube Publish (youtube)]**.
1. Sulla barra degli strumenti, a destra di Impostazioni, toccate **[!UICONTROL Modifica]**.
...
1. In the **[!UICONTROL YouTube Account Settings]** dialog box, in the **[!UICONTROL Application Name]** field, enter the Google Project ID.
   ...
   Lasciate aperta la finestra di dialogo Impostazioni **[!UICONTROL account]** YouTube; ritornerete su di esso tra un momento.
...
1. Return to the **[!UICONTROL YouTube Account Settings]** dialog box. Nel campo **[!UICONTROL Configurazione JSON]**, incolla il testo JSON.
1. Toccate **[!UICONTROL OK]**.
...
1. **Impostazione dei tag per la pubblicazione** - Nella pagina **[!UICONTROL Cloud Services > YouTube]** , toccate l’icona **[!UICONTROL Matita]** per modificare l’elenco dei tag da usare.
...
1. Nella pagina Editor **[!UICONTROL schema]** metadati, toccate la scheda **[!UICONTROL Avanzate]** .
1. Under the YouTube Publishing heading, tap **[!UICONTROL YouTube Category]**. (Non toccate l’elenco a discesa Categoria YouTube.)
1. Sul lato destro della pagina, nella scheda **[!UICONTROL Impostazioni]** , effettuate le seguenti operazioni:

   * Nel campo di testo Etichetta **** campo, selezionare e copiare il valore.
...
1. Under the YouTube Publishing heading, tap **[!UICONTROL YouTube Privacy]**. (Non toccate l’elenco a discesa Privacy di YouTube.)
1. Sul lato destro della pagina, nella scheda **[!UICONTROL Impostazioni]** , effettuate le seguenti operazioni:
...
1. Aggiungete i valori di Pubblicazione su YouTube copiati al profilo effettuando le seguenti operazioni:
   ...
   * Sul lato destro della pagina, toccate la scheda **[!UICONTROL Genera modulo]** .
   * Trascinare il componente **[!UICONTROL Intestazione]** sezione a sinistra e rilasciarlo nell’area del modulo.
   * Tap **[!UICONTROL Field Label]** to select the component.
   * Sul lato destro della pagina, nella scheda **[!UICONTROL Impostazioni]** , nel campo di testo Etichetta **** campo, immettere `YouTube Publishing`.
...
1. Nella barra degli strumenti, toccate **[!UICONTROL Proprietà]**.
1. Nella scheda **[!UICONTROL Base]** , nell’intestazione Metadati, toccate **[!UICONTROL Sfoglia]** a destra del campo **[!UICONTROL Tag]** .
1. Nella pagina **[!UICONTROL Seleziona tag]** , individuate i tag da usare e selezionate uno o più tag.
1. Nell’angolo superiore destro della pagina, toccate l’icona **[!UICONTROL Conferma]** .
1. In the upper-right corner of the video&#39;s properties page, tap **[!UICONTROL Save]**.
1. Nella barra degli strumenti, toccate **[!UICONTROL Pubblica > Pubblica]**.
...
1. Andate alla risorsa video *pubblicata* su YouTube di cui desiderate copiare l’URL, quindi selezionatela.
...
1. In **[!UICONTROL Pubblicazione]** su YouTube, nell’elenco URL **[!UICONTROL di]** YouTube, selezionate e copiate il testo dell’URL nel browser Web per visualizzare l’anteprima della risorsa o per aggiungerlo alla pagina del contenuto Web.
...
1. Nella barra degli strumenti, toccate **[!UICONTROL Annulla pubblicazione > Annulla pubblicazione]**.
...
1. Visualizzate l’avanzamento della codifica video nella cartella delle risorse:
   ...
   * Nella vista **[!UICONTROL a]** schede, l’avanzamento della codifica video viene visualizzato per percentuale sulla risorsa. In caso di errore, queste informazioni vengono visualizzate anche sulla risorsa.

      ![chlimage_1-429](assets/chlimage_1-429.png)

   * In **[!UICONTROL List View]**, video encoding progress displays in the **[!UICONTROL Processing Status]** column. In caso di errore, il messaggio viene visualizzato nella stessa colonna.

      ![chlimage_1-430](assets/chlimage_1-430.png)

      Questa colonna non viene visualizzata per impostazione predefinita. To enable the column, select **[!UICONTROL View Settings]** from the **[!UICONTROL Views]** drop-down menu, and add the **[!UICONTROL Processing Status]** column and tap **[!UICONTROL Update]**.
...
1. For workflows in progress, see **Workflow Instances** available from **[!UICONTROL Tools > Workflow > Instances]**.
   ...
   Selezionate l’istanza e toccate **[!UICONTROL Apri cronologia]**.
...
1. Nella pagina Rapporti video, effettuate una delle seguenti operazioni:
   ...
   * Nell&#39;angolo superiore destro, toccate l&#39;icona **[!UICONTROL Selezione]** data.
   ...
   La casella del gruppo Metriche **** principali identifica diverse misurazioni aggregate per tutti i video *pubblicati* sul sito.
...
1. Usate un’applicazione o un servizio di terze parti per creare il file di sottotitoli o sottotitoli video.
   ...
   In the tool, in the **[!UICONTROL Enter URL of video file]** field, paste the copied URL of your video file and then tap **[!UICONTROL Load]**. Consulta la sezione [Ottenere l’URL per una risorsa](linking-urls-to-yourwebapplication.md#obtaining-a-url-for-an-asset) per conoscere l’URL del file video stesso, che potrai incollare nel campo **[!UICONTROL Enter URL of video file (Inserisci URL del file video)]**. A quel punto, Internet Explorer, Chrome o Safari possono riprodurre il video in modalità nativa.
...
1. Salvate il `.vtt` file in codifica UTF8 per evitare problemi con la rappresentazione dei caratteri nel titolo del capitolo.
   ...
   <table> 
     <tbody> 
      ...
      <tr> 
       ...
       <li>Dal menu a discesa, toccate <strong>Visualizzatori</strong>.</li> 
       <li>Nella barra a sinistra, toccate il nome del predefinito per visualizzatori video. L’anteprima del video viene aperta in una pagina separata.</li> 
       <li>Nella barra a sinistra, nella parte inferiore, toccate <strong>Incorpora</strong>.</li> 
...
1. Nell’angolo in alto a sinistra della pagina dei CRXDE Lite, toccate **[!UICONTROL Salva tutto]**, quindi toccate l’icona **[!UICONTROL Indietro Home]** nell’angolo in alto a sinistra per tornare a AEM.
...
1. Sulla barra degli strumenti, toccate l’icona **[!UICONTROL Visualizza proprietà]** (un cerchio contenente un &quot;i&quot;).
1. Nella pagina **[!UICONTROL Proprietà]** del video, toccate **[!UICONTROL Cambia miniatura]**.
1. Nella pagina **[!UICONTROL Modifica miniatura]** , nella barra degli strumenti toccate **[!UICONTROL Carica nuova miniatura]**.
1. Passate all’immagine della miniatura da usare, selezionatela, quindi toccate **[!UICONTROL Apri]** per iniziare a caricare l’immagine in AEM
1. Dopo il corretto caricamento dell’immagine, nella pagina **[!UICONTROL Modifica miniatura]** toccate **[!UICONTROL Salva modifiche]**.
