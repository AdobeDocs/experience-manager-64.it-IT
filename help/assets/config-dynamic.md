...
seo-description: Scopri come configurare Dynamic Media - Modalità ibrida.
...
Dynamic Media: l’ibrido deve essere abilitato e configurato per l’uso. A seconda del caso d’uso, Dynamic Media dispone di diverse [configurazioni supportate](#supported-dynamic-media-configurations).
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td> 
    <ol> 
     <li>Sul nodo AEM <strong>author</strong>, <a href="#enabling-dynamic-media">abilita dynamic media</a>.</li> 
  ...
  <tr> 
   ...
   <td> 
    <ol> 
     <li>Sul nodo AEM <strong>author</strong>, <a href="#enabling-dynamic-media">abilita dynamic media</a>.</li> 
     <li>Sul nodo AEM <strong>pubblica</strong>, <a href="#enabling-dynamic-media">abilita dynamic media</a>.</li> 
     <li><a href="#replicating-viewer-presets">Replicare i predefiniti visualizzatore</a>.</li> 
  ...
  <tr> 
   ...
   <td> 
    <ol> 
     <li>Sul nodo AEM <strong>author</strong>, <a href="#enabling-dynamic-media">abilita dynamic media</a>.</li> 
     <li>Sul nodo AEM <strong>publish</strong>, <a href="#enabling-dynamic-media">abilita dynamic media</a> (l'istanza di pubblicazione serve l'immagine poster video e fornisce metadati per la riproduzione video).</li> 
...
[Dynamic ](https://www.adobe.com/solutions/web-experience-management/dynamic-media.html) Media disattivato per impostazione predefinita. Per sfruttare le funzioni di Dynamic Media, è necessario abilitare i contenuti multimediali dinamici utilizzando la modalità di esecuzione **[!UICONTROL dynamic media]** come si farebbe, ad esempio, con la modalità di esecuzione **[!UICONTROL publish]** . Prima di attivare, controlla i [requisiti tecnici](/help/sites-deploying/technical-requirements.md#requirements-for-aem-dynamic-media-add-on).

>[!NOTE]
>
>L’abilitazione degli elementi multimediali dinamici tramite la modalità di esecuzione sostituisce le funzionalità di AEM 6.1 e AEM 6.0 in cui è stato abilitato l’elemento multimediale dinamico impostando il flag **[!UICONTROL dynamicMediaEnabled]** su **[!UICONTROL true]**. Questo flag non ha funzionalità in AEM 6.2 e versioni successive. Inoltre, non è necessario riavviare l&#39;avvio rapido per abilitare gli elementi multimediali dinamici.

Attivando Dynamic Media, le funzioni per i file multimediali dinamici saranno disponibili nell’interfaccia utente e ogni risorsa immagine caricata riceverà un rendering `cqdam.pyramid.tiff` utilizzato per la distribuzione rapida di rappresentazioni dinamiche delle immagini. Tali PTIFF presentano vantaggi significativi, tra cui (1) la possibilità di gestire una sola immagine master e generare al volo rappresentazioni infinite senza ulteriore spazio di archiviazione e (2) la possibilità di utilizzare visualizzazioni interattive quali zoom, pan, spin e così via.

Se desideri utilizzare Dynamic Media Classic in AEM, non devi abilitare Dynamic Media a meno che tu non utilizzi un [scenario specifico](/help/sites-administering/scene7.md#aem-scene-integration-versus-dynamic-media). Dynamic Media è disattivato a meno che non si attivi Dynamic Media in modalità runmode.
...
**Per attivare gli elementi multimediali** dinamici:

1. Nella riga di comando, quando si avvia l&#39;avvio rapido, procedi come segue:

   * Aggiungi **[!UICONTROL -r dinamicmedia]** alla fine della riga di comando all&#39;avvio del file jar.
...
1. Richiedi `http://localhost:4502/is/image` e assicurati che Image Server sia in esecuzione.

   >[!NOTE]
   >
   >Per risolvere i problemi con Dynamic Media, vedi i seguenti registri nella directory **[!UICONTROL crx-quickstart/logs/]** :
   >
...
In una distribuzione WAR QuickStart AEM non è possibile derivare il numero di porta e il percorso contestuale, pertanto è necessario configurare un dominio **self**. Consulta la documentazione [esternalizzatore](/help/sites-developing/externalizer.md) su come configurare il dominio **self** .
...
Per disattivare i file multimediali dinamici dopo averlo abilitato, rimuovi il flag di modalità di esecuzione **[!UICONTROL -r dynamic]** .
...
1. Richiesta `http://localhost:4502/is/image`. Viene visualizzato un messaggio che informa che Dynamic Media è disattivato.
   ...
   >[!NOTE]
   Una volta disattivata la modalità di esecuzione di Dynamic Media, il passaggio del flusso di lavoro che genera il rendering `qdam.pyramid.tiff` viene ignorato automaticamente. Questo disattiva anche il supporto del rendering dinamico e altre funzioni di Dynamic Media.
...
Per modificare il limite di memoria per la creazione PTIFF, accedi a **[!UICONTROL Strumenti > Operazioni > Console web > Adobe CQ Scene7 PTiffManager]** e modifica il valore `maxMemory`.

### Impostazione dell&#39;autenticazione {#setting-up-authentication}

Per replicare le immagini nel servizio di distribuzione delle immagini Dynamic Media, è necessario impostare l’autenticazione della replica sull’autore. Per farlo, ottieni un KeyStore e poi lo salvi sotto l&#39;utente **[!UICONTROL dynamic-media-replication]** e configuralo. L’amministratore dell’azienda deve aver ricevuto un messaggio e-mail di benvenuto con il file KeyStore e le credenziali necessarie durante il processo di provisioning. In caso contrario, contatta l’Assistenza clienti.

**Per impostare l’autenticazione**:

1. Contatta l’Assistenza clienti per conoscere il tuo file KeyStore e la tua password, se non lo hai già. Fa parte del provisioning e associa le chiavi al tuo account.
...
1. Nella pagina Modifica impostazioni utente per la replica Dynamic-media, tocca la scheda **[!UICONTROL Registro chiavi]** , quindi tocca **[!UICONTROL Crea archivio chiavi]**.
...
1. Immetti una password e conferma la password nella finestra di dialogo **[!UICONTROL Imposta password di accesso KeyStore]** .
   ...
   >[!NOTE]
   Ricordare la password immessa. Sarà necessario immetterlo nuovamente quando configurerai l&#39; **[!UICONTROL Agente di replica]** in un secondo momento.

   ![chlimage_1-508](assets/chlimage_1-508.png)

1. Nella pagina **[!UICONTROL Modifica impostazioni utente per dynamic-media-replication]** , espandi l&#39;area **[!UICONTROL Aggiungi chiave privata dal file KeyStore]** e aggiungi quanto segue (vedi le immagini che seguono):

   * Nel campo **[!UICONTROL Nuovo alias]**, immetti il nome di un alias che utilizzerai successivamente nella configurazione di replica; ad esempio, **replica**.
   * Tocca **[!UICONTROL File KeyStore]**. Passa al file KeyStore fornito per Adobe, selezionalo, quindi tocca **[!UICONTROL Apri]**.
   * Nel campo **[!UICONTROL Password file KeyStore]** , immetti la password del file KeyStore. Si tratta di _not_ la password KeyStore che hai creato nel passaggio 5, ma è l&#39;Adobe di password del file KeyStore fornito nel messaggio e-mail di benvenuto che ti è stato inviato durante il provisioning. Se non hai ricevuto una password del file KeyStore, contatta l’Assistenza clienti Adobe.
   * Nel campo **[!UICONTROL Password chiave privata]** , immetti la password della chiave privata (potrebbe essere la stessa password della chiave privata fornita nel passaggio precedente). Adobe fornisce la password della chiave privata nell’e-mail di benvenuto inviata durante il provisioning. Se non hai ricevuto una password di chiave privata, contatta l’Assistenza clienti di Adobe.
...
1. In AEM, tocca il logo AEM per accedere alla console di navigazione globale, quindi tocca **[!UICONTROL Strumenti > Implementazione > Replica > Agenti su autore]**.
...
1. Tocca la scheda **[!UICONTROL Impostazioni]** , quindi immetti quanto segue:
   ...
   * **[!UICONTROL ID tenant]** : questo valore è il nome della società/tenant che esegue la pubblicazione nel servizio di replica. Questo valore è l’ID tenant fornito in Adobe nell’e-mail di benvenuto inviata durante il provisioning. Se non hai ricevuto questa richiesta, contatta l’Assistenza clienti Adobe.
   ...
   * **[!UICONTROL Password archivio chiavi]** : password KeyStore creata quando hai toccato  **[!UICONTROL Create KeyStore]**. Adobe non fornisce questa password. Vedi il passaggio 5 di [Configurazione dell&#39;autenticazione](#setting-up-authentication).
...
* Pubblica un’immagine. Tocca l’immagine e seleziona **[!UICONTROL Visualizzatori]** nel menu a discesa. Seleziona un predefinito per visualizzatori, quindi tocca **[!UICONTROL URL]** e copia e incolla l’URL nel browser per verificare che sia possibile visualizzare l’immagine.
...
Questo problema è causato da un errore di configurazione nell’istanza di AEM Author. Il processo java sull&#39;autore non riceve il `javax.net.ssl.trustStore` corretto. Questo errore viene visualizzato nel registro di replica:
...
1. Passa alla pagina **[!UICONTROL Gestione utente]** :

   `localhost:4502/libs/granite/security/content/useradmin.html`
1. Nella pagina **[!UICONTROL Gestione utente]** , individua l&#39;utente **[!UICONTROL dynamic-media-replication]** , quindi tocca per aprire.
1. Tocca la scheda **[!UICONTROL KeyStore]** . Se viene visualizzato il pulsante **[!UICONTROL Crea KeyStore]**, è necessario ripristinare i passaggi precedenti in [Impostazione dell&#39;autenticazione](#setting-up-authentication) .
1. Se è stato necessario ripristinare la configurazione di **[!UICONTROL KeyStore]**, potrebbe essere necessario eseguire di nuovo anche la configurazione di [Replication Agent](config-dynamic.md#configuring-the-replication-agent).
...
1. Passa al nodo **[!UICONTROL s7delivery Replication Agent]** .
...
* Configura **Configurazione Dynamic Media (Pre 6.3)** in **[!UICONTROL Cloud Services]**. L’URL del servizio immagini è necessario per questo test
...
**Per configurare i servizi** cloud per elementi multimediali dinamici:
...
1. Nella pagina **[!UICONTROL Browser configurazione Dynamic Media]**, seleziona **[!UICONTROL globale]** nel riquadro a sinistra, quindi tocca **[!UICONTROL Crea]**.
...
**Quando utilizzare:** al momento della configurazione della configurazione di  **[!UICONTROL Dynamic Media (prima 6.3)]**, vengono avviate numerose funzioni, tra cui il reporting video. La configurazione crea una suite di rapporti in una società Analytics regionale. Se configuri più nodi Autore, crea una suite di rapporti separata per ciascuno di essi. Di conseguenza, i dati di segnalazione sono incoerenti tra le installazioni. Inoltre, se ogni nodo Autore fa riferimento allo stesso server di pubblicazione ibrida, l’ultima installazione Autore modifica la suite di rapporti di destinazione per tutti i rapporti video. Questo problema sovraccarica il sistema Analytics con troppe suite di rapporti.

**Introduzione:** configura il reporting video completando le tre attività seguenti.

1. Crea un pacchetto predefinito [!DNL Video Analytics] dopo aver configurato **[!UICONTROL Configurazione Dynamic Media (Pre 6.3)]** sul primo nodo Autore. Questa attività iniziale è importante perché consente a una nuova configurazione di continuare a utilizzare la stessa suite di rapporti.
1. Installa il pacchetto predefinito [!DNL Video Analytics] su qualsiasi ***nuovo*** nodo Autore ***prima*** di configurare la configurazione Dynamic Media (Pre 6.3).

1. Verifica ed esegui il debug dell&#39;installazione del pacchetto.

### Creazione di un pacchetto predefinito [!DNL Video Analytics] dopo la configurazione del primo nodo Autore {#creating-a-video-analytics-preset-package-after-configuring-the-first-author-node}

Al termine di questa attività, avrai a disposizione un file di pacchetto contenente i predefiniti [!DNL Video Analytics] . Questi predefiniti contengono una suite di rapporti, il server di tracciamento, lo spazio dei nomi di tracciamento e l’ID organizzazione del Marketing Cloud, se disponibile.

1. Se non lo hai già fatto, configura **[!UICONTROL Configurazione Dynamic Media (Pre 6.3)]**.
1. (Facoltativo) Visualizza e copia l&#39; **[!UICONTROL ID suite di rapporti]** (devi disporre dell&#39;accesso a JCR). Anche se non è necessario disporre dell&#39; **[!UICONTROL ID suite di rapporti]**, la convalida risulta più semplice.
...
1. Crea il pacchetto.
1. Scarica o condividi il pacchetto predefinito [!DNL Video Analytics] in modo che possa essere condiviso con i nuovi nodi Autore successivi.

### Installazione del pacchetto predefinito [!DNL Video Analytics] prima di configurare altri nodi Author {#installing-the-video-analytics-preset-package-before-you-configure-additional-author-nodes}

Assicurati di completare questa attività _prima_ di configurare **[!UICONTROL Configurazione Dynamic Media (Pre 6.3)]**. In caso contrario, verrà creata un’altra suite di rapporti non utilizzata. Inoltre, anche se il reporting video continuerà a funzionare correttamente, la raccolta dei dati non viene ottimizzata.

Assicurati che il pacchetto predefinito [!DNL Video Analytics] dal primo nodo Autore sia accessibile sul nuovo nodo Autore.

1. Carica il pacchetto predefinito [!DNL Video Analytics] creato in precedenza in **[!UICONTROL Gestione pacchetti]**.
...
1. Esegui una delle seguenti operazioni per verificare e, se necessario, eseguire il debug dell&#39;installazione del pacchetto:
   ...
   * **Controlla il  [!DNL Video Analytics] predefinito tramite**
JCRTo controlla il  [!DNL Video Analytics] predefinito tramite JCR, devi avere accesso a  **[!UICONTROL CRXDE Lite]**.

      AEM - In **[!UICONTROL CRXDE Lite]**, passa a `/conf/global/settings/dam/dm/presets/analytics/jcr:content/userdata  `

      ovvero `http://localhost:4502/crx/de/index.jsp#/conf/global/settings/dam/dm/presets/analytics/jcr%3Acontent/userdata`

      Se non hai accesso a **[!UICONTROL CRXDE Lite]** sul nodo Autore, puoi controllare il predefinito tramite il server Publish.

   * **Controlla il  [!DNL Video Analytics] predefinito tramite il server di immagini**

      Puoi convalidare il predefinito [!DNL Video Analytics] direttamente effettuando una richiesta Image Server `req=userdata`.

      Ad esempio, per visualizzare il predefinito [!DNL Video Analytics] sul nodo Autore, puoi effettuare la seguente richiesta:
   ...
   * **Controlla il  [!DNL Video Analytics] predefinito tramite lo strumento di reporting video in AEM**
   ...
   Questo errore viene visualizzato anche se Video Reporting viene eseguito prima di configurare i servizi **[!UICONTROL Configurazione Dynamic Media (Pre 6.3)]** .
...
* La mancata installazione del pacchetto predefinito [!DNL Video Analytics] può causare la creazione di una nuova suite di rapporti.
* L&#39;aggiornamento da AEM 6.3 a AEM 6.4 o AEM 6.4.1, quindi la configurazione di **[!UICONTROL Configurazione Dynamic Media (Pre 6.3)]**, crea ancora una suite di rapporti. Questo problema è noto e dovrebbe essere risolto per AEM 6.4.2.

### Informazioni sul predefinito [!DNL Video Analytics] {#about-the-video-analytics-preset}

Il predefinito [!DNL Video Analytics], talvolta noto semplicemente come predefinito di analisi, viene memorizzato accanto ai predefiniti visualizzatore in Dynamic Media. È sostanzialmente lo stesso di un predefinito per visualizzatori, ma con le informazioni utilizzate per configurare il reporting di AppMeasurement e Video Heartbeat.
...
* **[!UICONTROL marketingCloudOrgId]**  (non presente nelle versioni precedenti di AEM)
...
1. In AEM, accedi alla seguente posizione in **[!UICONTROL CRXDE Lite]** (richiede privilegi di amministratore):
...
>[!NOTE]
Per impostazione predefinita, il sistema mostra una varietà di rappresentazioni quando selezioni **[!UICONTROL Rappresentazioni]** e una varietà di predefiniti visualizzatore quando selezioni **[!UICONTROL Visualizzatori]** nella visualizzazione dei dettagli della risorsa. È possibile aumentare o diminuire il numero visualizzato. Consulta [Aumento del numero di predefiniti immagine da visualizzare](/help/assets/managing-image-presets.md#increasing-or-decreasing-the-number-of-image-presets-that-display) o [Aumento del numero di predefiniti visualizzatore da visualizzare](/help/assets/managing-viewer-presets.md#increasing-the-number-of-viewer-presets-that-display).

## Filtrare le risorse per la replica {#filtering-assets-for-replication}

Nelle implementazioni non Dynamic Media, replichi tutte le risorse _tutte_ (sia immagini che video) dall’ambiente di authoring AEM al nodo di pubblicazione AEM. Questo flusso di lavoro è necessario perché i server di pubblicazione AEM distribuiscono anche le risorse.
...
I filtri ti consentono di _escludere_ risorse dalla replica nel nodo di pubblicazione AEM.
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>Puoi configurare l’URI di trasporto in modo che punti al server di pubblicazione AEM invece dell’URL del servizio Adobe Dynamic Media Cloud Replication Service. L’impostazione di questo filtro consentirà a Dynamic Media Classic di distribuire le risorse invece dell’istanza di pubblicazione AEM.</p> <p>I filtri-immagine, i "set-filtri" e i "filtri-video" preconfigurati:</p> 
    <ul> 
     <li>Includi immagini PTIFF, rappresentazioni video proxy e metadati per la replica. Tuttavia, poiché non esistono nel JCR-per coloro che eseguono AEM - integrazione Dynamic Media Classic - non fa nulla.</li> 
     <li>Escludere dalla replica l'immagine originale, le rappresentazioni statiche dell'immagine, il video originale e le rappresentazioni statiche delle miniature. Al contrario, Dynamic Media Classic fornirà risorse immagine e video.</li> 
    </ul> </td> 
  </tr> 
 </tbody> 
</table>
...
1. In AEM, tocca il logo AEM per accedere alla console di navigazione globale e tocca **[!UICONTROL Strumenti > Implementazione > Replica > Agenti su autore]**.
...
1. Nella struttura ad albero delle cartelle a sinistra, passa a `/etc/replication/agents.author/publish`
1. Individua **[!UICONTROL jcr:content]**, fai clic con il pulsante destro del mouse su di esso e seleziona **[!UICONTROL Crea > Crea nodo]**. Immettere il nome `damRenditionFilters` di tipo `nt:unstructured`.
...
Se utilizzi più di un filtro su un server, ad esempio un filtro per la pubblicazione della replica e un secondo filtro per s7delivery, devi accertarti che a questi due filtri sia assegnato un **userId** diverso nel nodo **[!UICONTROL jcr:content]** . Vedi l&#39;immagine che segue:
...
1. Per definire il tipo di MIME per il filtro, potete individuare il tipo di MIME come segue:
   ...
   Nella barra a sinistra, espandi **[!UICONTROL contenuto > dam > &lt;`locate_your_asset`> > jcr:content > metadata]**, quindi individua `dc:format` nella tabella.
...
1. Nella pagina **[!UICONTROL Configurazione console Web Adobe Experience Manager]**, tocca **[!UICONTROL OSGi > Configurazione]** per elencare tutti i bundle attualmente in esecuzione in AEM.
...
1. Nell&#39;elenco dei bundle, a destra di **[!UICONTROL Adobe CQ Scene7 ImageServer]**, tocca l&#39;icona **[!UICONTROL Modifica]**.
1. Nella finestra di dialogo **[!UICONTROL Adobe CQ Scene7 ImageServer]**, imposta i seguenti valori di configurazione:
...
1. Nell&#39;elenco dei bundle, a destra di **[!UICONTROL Adobe CQ Scene7 PlatformServer]**, tocca l&#39;icona **[!UICONTROL Modifica]**.
1. Nella finestra di dialogo **[!UICONTROL Adobe CQ Scene7 PlatformServer]**, imposta le seguenti opzioni di valore predefinite:
...
Il manifesto predefinito ti consente di configurare i valori predefiniti utilizzati per generare le risposte di Dynamic Media Delivery. È possibile ottimizzare la qualità (qualità JPEG, risoluzione, modalità di ricampionamento), la memorizzazione in cache (scadenza) e impedire il rendering di immagini troppo grandi (valori predefiniti, defaultthumbpix, maxpix).
...
Una volta apportate le modifiche al manifesto predefinito, tocca **[!UICONTROL Salva tutto]** nell’angolo in alto a sinistra della pagina.

Tocca la scheda **[!UICONTROL Controllo accessi]** (a destra della scheda **[!UICONTROL Proprietà]**), quindi imposta i privilegi di controllo accessi su `jcr:read` per tutti gli utenti e gli utenti di replica di elementi multimediali dinamici.
...
È possibile configurare la gestione del colore per gli elementi multimediali dinamici e configurare i predefiniti per immagini utilizzando l’output CMYK, RGB o Grigio. Consulta [Configurazione dei predefiniti per immagini](managing-image-presets.md).

I casi d&#39;uso avanzati possono utilizzare un modificatore di configurazione manuale **[!UICONTROL icc=]** per selezionare esplicitamente un profilo colore di output:
...
1. Aggiungi una proprietà di correzione del colore scorrendo nella parte inferiore della scheda **[!UICONTROL Proprietà]** e immettendo manualmente il nome, il tipo e il valore della proprietà, descritti nelle tabelle seguenti. Dopo aver inserito i valori, tocca **[!UICONTROL Aggiungi]** e quindi **[!UICONTROL Salva tutto]** per salvare i valori.

   Le proprietà di correzione del colore sono descritte nella tabella **[!UICONTROL Proprietà correzione colore]**. I valori che è possibile assegnare alle proprietà di correzione del colore si trovano nella tabella **[!UICONTROL Profilo colore]**.
...
Ad esempio, puoi impostare **[!UICONTROL iccprofilergb]** su `sRGB` e **[!UICONTROL iccprofilecmyk]** su `WebCoated`. In questo modo si effettua quanto segue:
...
Dopo aver completato tutte le attività di cui sopra, le risorse Dynamic Media attivate vengono servite da Image o Video Service. In AEM, questa funzionalità viene visualizzata in un **[!UICONTROL Copia URL immagine]**, **[!UICONTROL Copia URL visualizzatore]**, **[!UICONTROL Incorpora codice visualizzatore]** e nel WCM.

Consulta [Distribuzione di risorse Dynamic Media](delivering-dynamic-media-assets.md).
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>La finestra di dialogo Copia URL visualizza un URL simile al seguente (l’URL è a scopo dimostrativo):</p> <p><code>https://PUBLISHNODE/etc/dam/viewers/s7viewers/html5/BasicZoomViewer.html?asset=/content/dam/path/to/Image.jpg&amp;config=/conf/global/settings/dam/dm/presets/viewer/Zoom_dark&amp;serverUrl=https://IMAGESERVICEPUBLISHNODE/is/image/&amp;contentRoot=%2F</code></p> <p>Dove <code>PUBLISHNODE</code> fa riferimento al normale nodo di pubblicazione AEM e <code>IMAGESERVICEPUBLISHNODE</code> fa riferimento all’URL del servizio immagine.</p> <p>Consulta anche <a href="/help/assets/delivering-dynamic-media-assets.md">Distribuzione di risorse Dynamic Media</a>.</p> </td> 
  </tr> 
  <tr> 
   ...
   <td><p>La finestra di dialogo Copia codice di incorporamento visualizza uno snippet di codice simile al seguente (l'esempio di codice è solo a scopo dimostrativo):</p> <p><code class="code">&lt;style type="text/css"&gt;
       ...
       &lt;/script&gt;</code></p> <p>Dove <code>PUBLISHNODE</code> fa riferimento al normale nodo di pubblicazione AEM e <code>IMAGESERVICEPUBLISHNODE</code> fa riferimento all’URL del servizio immagine.</p> <p>Consulta anche <a href="/help/assets/delivering-dynamic-media-assets.md">Distribuzione di risorse Dynamic Media</a>.</p> </td> 
  </tr> 
 </tbody> 
</table>
