...
>[!NOTE]
>
>Se si intende configurare ed eseguire Dynamic Media in modalità di esecuzione Scene7, vedere [Configurazione di Dynamic Media - Modalità Scene7](config-dms7.md).
>
...
[Per impostazione predefinita, i ](https://www.adobe.com/solutions/web-experience-management/dynamic-media.html) contenuti multimediali dinamici sono disattivati. Per sfruttare le funzionalità di Dynamic Media, è necessario abilitare i supporti dinamici utilizzando la modalità di esecuzione **[!UICONTROL dynamicmedia]**, come si fa ad esempio per la modalità di esecuzione **[!UICONTROL pubblica]**. Prima di eseguire l&#39;attivazione, verificare i [requisiti tecnici](/help/sites-deploying/technical-requirements.md#requirements-for-aem-dynamic-media-add-on).

>[!NOTE]
>
>L&#39;attivazione di supporti dinamici tramite la modalità di esecuzione sostituisce le funzionalità delle AEM 6.1 e AEM 6.0 in cui sono stati abilitati i supporti dinamici impostando il flag **[!UICONTROL dynamicMediaEnabled]** su **[!UICONTROL true]**. Questo flag non ha funzionalità in AEM 6.2 e versioni successive. Inoltre, non è necessario riavviare il servizio QuickStart per abilitare i contenuti multimediali dinamici.
...
1. Nella riga di comando, all’avvio dell’avvio rapido, effettuate le seguenti operazioni:
   ...
   * Aggiungete **[!UICONTROL -r dynamicmedia]** alla fine della riga di comando all&#39;avvio del file jar.
...
1. Richiedete `http://localhost:4502/is/image` e accertatevi che Image Server sia in esecuzione.
   ...
   >[!NOTE]
   >
   >Per risolvere i problemi con Dynamic Media, consultare i seguenti file di registro nella directory **[!UICONTROL crx-quickstart/logs/]**:
   >
   ...
   * s7access-&lt;yyyy>&lt;mm>&lt;dd>.log - Il registro di accesso s7access registra ogni richiesta effettuata ad Dynamic Media tramite `/is/image` e `/is/content`.
   Questi registri vengono utilizzati solo quando Dynamic Media è abilitato. non sono inclusi nel pacchetto **Download completo** generato dalla pagina **[!UICONTROL system/console/status-Bundlelist]**; quando chiamate l&#39;Assistenza clienti in caso di problemi con Dynamic Media, aggiungete entrambi i registri al problema.
...
Per disabilitare il supporto dinamico dopo averlo abilitato, rimuovere il flag di modalità di esecuzione **[!UICONTROL -r dynamicmedia]**.
...
1. Richiesta `http://localhost:4502/is/image`. Viene visualizzato un messaggio che informa che Dynamic Media è disattivato.
   ...
   >[!NOTE]
   Una volta disattivata la modalità di esecuzione di Dynamic Media, il passaggio del flusso di lavoro che genera il rendering `qdam.pyramid.tiff` viene ignorato automaticamente. In questo modo viene disattivato anche il supporto delle rappresentazioni dinamiche e altre funzionalità di Dynamic Media.
...
## (Facoltativo) Migrazione dei predefiniti e delle configurazioni Dynamic Media da 6.3 a 6.4 senza tempi di inattività {#optional-migrating-dynamic-media-presets-and-configurations-from-to-zero-downtime}

Se state effettuando l&#39;aggiornamento AEM Dynamic Media dalla versione 6.3 alla versione 6.4, che ora include la possibilità di installare senza tempi di inattività (altrimenti nota come &quot;consenso esplicito&quot;), dovete eseguire il seguente comando curl per migrare tutti i predefiniti e le configurazioni da `/etc` a `/conf` in CRXDE Lite.
...
Per modificare il limite di memoria per la creazione PTIFF, andate a **[!UICONTROL Strumenti > Operazioni > Console Web >  Adobe CQ Scene7 PTiffManager]** e modificate il valore `maxMemory`.
...
1. Nella pagina Edit User Settings For dynamic-media-replica, toccate la scheda **[!UICONTROL Keystore]**, quindi toccate **[!UICONTROL Create KeyStore]**.
...
1. Nella pagina **[!UICONTROL Edit User Settings For dynamic-media-replica]** (Modifica impostazioni utente per la replica Dynamic-media), espandere l&#39;area **[!UICONTROL Add Private Key from KeyStore file]** e aggiungere quanto segue (vedere le immagini che seguono):
   ...
   * Nel campo **[!UICONTROL Password file KeyStore]**, immettere la password del file KeyStore. Si tratta di _not_ la password KeyStore che avete creato nel passaggio 5, ma è il Adobe della password del file KeyStore  fornisce nel messaggio e-mail di benvenuto che vi è stato inviato durante il provisioning. Contatta  Assistenza clienti di Adobe se non hai ricevuto una password per il file KeyStore.
...
1. In AEM, toccate il logo AEM per accedere alla console di navigazione globale, quindi toccate **[!UICONTROL Strumenti > Distribuzione > Replica > Agenti sull&#39;autore]**.
...
Toccare **[!UICONTROL Test Connection]**. Esempio di output:
...
* Pubblicate un’immagine. Toccate l&#39;immagine e selezionate **[!UICONTROL Visualizzatori]** nel menu a discesa. Selezionate un predefinito per visualizzatori, quindi toccate **[!UICONTROL URL]**, quindi copiate e incollate l&#39;URL nel browser per verificare che sia possibile visualizzare l&#39;immagine.
...
**Soluzione**: Verificate che il file  `KeyStore` venga salvato in  **[!UICONTROL dynamic-media-]** replicationuser e che sia fornito con la password corretta.
...
**Soluzione**: Assicuratevi che la procedura Java in AEM Author contenga la proprietà di sistema  **-Djavax.net.ssl.trustStore=** impostata su un archivio attendibili valido.
...
1. Andate alla pagina **[!UICONTROL Gestione utente]**:

   `localhost:4502/libs/granite/security/content/useradmin.html`
1. Nella pagina **[!UICONTROL Gestione utente]**, andate all&#39;utente **[!UICONTROL replica-media dinamica]**, quindi toccate per aprirlo.
1. Toccate la scheda **[!UICONTROL KeyStore]**. Se viene visualizzato il pulsante **[!UICONTROL Create KeyStore]**, è necessario ripristinare i passaggi in [Setting up Authentication](#setting-up-authentication) (Impostazione dell&#39;autenticazione) in precedenza.
1. Se è stato necessario ripristinare la configurazione di **[!UICONTROL KeyStore]**, potrebbe essere necessario eseguire di nuovo la procedura [Configurazione dell&#39;agente di replica](config-dynamic.md#configuring-the-replication-agent).
...
1. Vicino all&#39;angolo superiore sinistro della pagina, toccare **[!UICONTROL Salva tutto]**.
...
* Configurare **Configurazione Dynamic Media (Pre 6.3)** in **[!UICONTROL Cloud Services]**. L’URL del servizio immagini è richiesto per questo test
...
>[!NOTE]
Prima di configurare i Cloud Services Dynamic Media, accertatevi che l’istanza di pubblicazione sia impostata. Prima di configurare i Cloud Services Dynamic Media è inoltre necessario disporre della replica impostata.
...
1. Nella pagina **[!UICONTROL Dynamic Media Configuration Browser]**, nel riquadro a sinistra selezionare **[!UICONTROL global]**, quindi toccare **[!UICONTROL Create]**.
...
1. Se state configurando Dynamic Media per il video,
   ...
   * Nel campo **[!UICONTROL URL servizio video]**, immettete l&#39;URL del servizio video per Dynamic Media Gateway.
...
Potete configurare il reporting video per più installazioni di AEM utilizzando la modalità ibrida Dynamic Media.
...
1. Create un pacchetto di predefiniti [!DNL Video Analytics] dopo aver configurato **[!UICONTROL Configurazione Dynamic Media (Pre 6.3)]** sul primo nodo Autore. Questa attività iniziale è importante perché consente a una nuova configurazione di continuare a utilizzare la stessa suite di rapporti.
1. Installate il pacchetto di predefiniti [!DNL Video Analytics] in qualsiasi ***nuovo nodo*** Autore ***prima di*** configurate la configurazione Dynamic Media (Pre 6.3).
...
Al termine dell&#39;attività, sarà disponibile un file di pacchetto contenente i predefiniti [!DNL Video Analytics]. Questi predefiniti contengono una suite di rapporti, il server di tracciamento, lo spazio dei nomi di tracciamento e l’ID organizzazione Marketing Cloud, se disponibile.

1. Se non lo avete ancora fatto, configurate **[!UICONTROL Configurazione Dynamic Media (Pre 6.3)]**.
1. (Facoltativo) Visualizzare e copiare l&#39; **[!UICONTROL ID suite di rapporti]** (è necessario avere accesso al JCR). Anche se l&#39;ID **[!UICONTROL Suite di rapporti]** non è obbligatorio, la convalida risulta più semplice.
...
1. Scaricate o condividete il pacchetto di predefiniti [!DNL Video Analytics] in modo che possa essere condiviso con i nuovi nodi Author successivi.

### Installazione del pacchetto di predefiniti [!DNL Video Analytics] prima di configurare altri nodi Author {#installing-the-video-analytics-preset-package-before-you-configure-additional-author-nodes}

Assicurarsi di completare l&#39;attività _prima di_ di configurare **[!UICONTROL Configurazione Dynamic Media (Pre 6.3)]**. In caso contrario, verrà creata un&#39;altra suite di rapporti inutilizzata. Inoltre, anche se il reporting video continuerà a funzionare correttamente, la raccolta di dati non è ottimizzata.

Accertatevi che il pacchetto [!DNL Video Analytics] predefinito del primo nodo Autore sia accessibile nel nuovo nodo Autore.

1. Caricate il pacchetto [!DNL Video Analytics] predefinito creato in precedenza in **[!UICONTROL Package Manager]**.
1. Installate il pacchetto di predefiniti [!DNL Video Analytics].
1. Configurare **[!UICONTROL Configurazione Dynamic Media (Pre 6.3)]**.
...
1. Per verificare e, se necessario, eseguire il debug dell&#39;installazione del pacchetto, effettuate una delle seguenti operazioni:
   ...
   * **Controllate il  [!DNL Video Analytics] predefinito tramite**
JCRTper controllare il  [!DNL Video Analytics] predefinito tramite JCR, dovete avere accesso al  **[!UICONTROL CRXDE Lite]**.

      AEM - In **[!UICONTROL CRXDE Lite]**, passare a `/conf/global/settings/dam/dm/presets/analytics/jcr:content/userdata  `

      ovvero `http://localhost:4502/crx/de/index.jsp#/conf/global/settings/dam/dm/presets/analytics/jcr%3Acontent/userdata`

      Se non disponete dell&#39;accesso a **[!UICONTROL CRXDE Lite]** nel nodo Autore, potete controllare il predefinito tramite il server di pubblicazione.

   * **Controllare il  [!DNL Video Analytics] predefinito tramite il server immagini**

      Potete convalidare il predefinito [!DNL Video Analytics] direttamente effettuando una richiesta per Image Server `req=userdata`.

      Ad esempio, per visualizzare il predefinito [!DNL Video Analytics] nel nodo Autore, potete effettuare la seguente richiesta:
   ...
   * **Controllate il  [!DNL Video Analytics] predefinito mediante lo strumento di reporting video in AEM**
      ...
      Toccare **[!UICONTROL Strumenti > Risorse > Video Reporting]** `http://localhost:4502/mnt/overlay/dam/gui/content/s7dam/videoreports/videoreport.html`
...
* Se prima non si installa il pacchetto di predefiniti [!DNL Video Analytics], è possibile creare una nuova suite di rapporti.
* L&#39;aggiornamento da AEM 6.3 a AEM 6.4 o AEM 6.4.1, quindi la configurazione di **[!UICONTROL Dynamic Media Configuration (Pre 6.3)]**, crea comunque una suite di rapporti. Questo problema è noto e dovrebbe essere risolto per AEM 6.4.2.

### Informazioni sul predefinito [!DNL Video Analytics] {#about-the-video-analytics-preset}

Il predefinito [!DNL Video Analytics], noto anche semplicemente come predefinito di analisi, viene memorizzato accanto ai predefiniti per visualizzatori in Dynamic Media. È sostanzialmente lo stesso di un predefinito per visualizzatori, ma con le informazioni utilizzate per configurare il reporting AppMeasurement e Video Heartbeat.
...
1. In AEM, andate al seguente percorso in **[!UICONTROL CRXDE Lite]** (richiede privilegi di amministratore):
...
Per distribuire una risorsa con un predefinito per visualizzatori, dovete replicare o pubblicare il predefinito per visualizzatori. Tutti i predefiniti per visualizzatori devono essere attivati _e_ replicati per ottenere l’URL o il codice da incorporare per una risorsa. Per ulteriori informazioni, consultate [Predefiniti visualizzatore pubblicazione](managing-viewer-presets.md#publishing-viewer-presets).
...
Nelle distribuzioni non Dynamic Media, potete replicare tutte le risorse _tutte_ (immagini e video) dall’ambiente di authoring AEM al nodo di pubblicazione AEM. Questo flusso di lavoro è necessario perché i server di pubblicazione AEM forniscono anche le risorse.
...
I filtri forniscono un modo per escludere le risorse _escludendo_ dalla replica nel nodo di pubblicazione AEM.

### Utilizzo dei filtri risorse predefiniti per la replica {#using-default-asset-filters-for-replication}

Se si utilizza Dynamic Media per 1) imaging in produzione di immagini e video _o_ 2), è possibile utilizzare i filtri predefiniti forniti così come sono. Per impostazione predefinita, sono attivi i seguenti filtri:
...
1. In AEM, toccate il logo AEM per accedere alla console di navigazione globale e toccate **[!UICONTROL Strumenti > Distribuzione > Replica > Agenti sull&#39;autore]**.
...
### Impostazione dei filtri risorse per l&#39;imaging nelle distribuzioni non di produzione {#setting-up-asset-filters-for-imaging-in-non-production-deployments}
...
1. Individuare **[!UICONTROL filter-images]**, fare clic con il pulsante destro del mouse su di esso e selezionare **[!UICONTROL Copia]**.
1. Nella struttura delle cartelle a sinistra, andate a `/etc/replication/agents.author/publish`
1. Individuare **[!UICONTROL jcr:content]**, fare clic con il pulsante destro del mouse su di esso e selezionare **[!UICONTROL Crea > Crea nodo]**. Immettere il nome `damRenditionFilters` di tipo `nt:unstructured`.
1. Individuare [!UICONTROL `damRenditionFilters`], fare clic con il pulsante destro del mouse su di esso e selezionare **[!UICONTROL Incolla]**.
...
Se si utilizzano più filtri su un server, ad esempio un filtro per la pubblicazione della replica e un secondo filtro per la distribuzione s7delivery, è necessario assicurarsi che a questi due filtri sia assegnato un **userId** diverso nel nodo **[!UICONTROL jcr:content]**. Guardate l’immagine che segue:
...
**Prerequisito**:  _Prima di_ configurare Dynamic Media Image Server, verificare che la VM di Windows includa un&#39;installazione delle librerie Microsoft Visual C++. Le librerie sono necessarie per eseguire Dynamic Media Image Server. È possibile [scaricare il pacchetto ridistribuibile di Microsoft Visual C++ 2010 (x64) qui](https://www.microsoft.com/en-us/download/details.aspx?id=14632).

**Per configurare le impostazioni** di Dynamic Media Image Server:

1. Nell&#39;angolo superiore sinistro di AEM, toccare **[!UICONTROL Adobe Experience Manager]** per accedere alla console di navigazione globale, quindi toccare **[!UICONTROL Strumenti > Operazioni > Console Web]**.
1. Nella pagina **[!UICONTROL Configurazione console Web Adobe Experience Manager]**, toccare **[!UICONTROL OSGi > Configurazione]** per elencare tutti i bundle attualmente in esecuzione in AEM.
...
1. Nell&#39;elenco dei bundle, a destra di **[!UICONTROL Adobe CQ Scene7 ImageServer]**, toccare l&#39;icona **[!UICONTROL Edit]**.
...
1. Nell&#39;elenco dei bundle, a destra di **[!UICONTROL Adobe CQ Scene7 PlatformServer]**, toccare l&#39;icona **[!UICONTROL Edit]**.
1. Nella finestra di dialogo **[!UICONTROL Adobe CQ Scene7 PlatformServer]**, impostare le seguenti opzioni di valore predefinite:
   ...
   | **[!UICONTROL Dimensione massima cache]** | 20000000 | Dimensione massima della cache della risposta in byte. |
...
Dopo aver apportato le modifiche al manifesto predefinito, toccate **[!UICONTROL Salva tutto]** nell&#39;angolo in alto a sinistra della pagina.

Toccate la scheda **[!UICONTROL Controllo accesso]** (a destra della scheda **[!UICONTROL Proprietà]**), quindi impostate i privilegi di controllo di accesso su `jcr:read` per tutti gli utenti e per gli utenti di replica per contenuti multimediali dinamici.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>Questi valori rappresentano un'istantanea del tempo di riproduzione video e vengono passati a <a href="https://encoding.com/">encoding.com</a>. Per ulteriori informazioni, vedere <a href="/help/assets/video.md#about-video-thumbnails">Le miniature video</a>.</td> 
  </tr> 
 </tbody> 
</table>
...
Potete configurare la gestione del colore per i contenuti multimediali dinamici e i predefiniti per immagini mediante l’output CMYK, RGB o Grigio. Consultate [Configurazione dei predefiniti per immagini](managing-image-presets.md).

Gli scenari d&#39;uso avanzati possono utilizzare il modificatore di configurazione manuale **[!UICONTROL icc=]** per selezionare esplicitamente un profilo colore di output:

* **[!UICONTROL icc]** - Profilo colore  [Output.](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/http-protocol-reference/command-reference/r-icc.html)

* **[!UICONTROL iccEmbed]** -  [Incorpora profilo colore.](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/http-protocol-reference/command-reference/r-iccembed.html)
...
1. In **[!UICONTROL Strumenti > Generale > CRXDE Lite]**, passare a `/conf/global/settings/dam/dm/imageserver/configuration/settings` che contiene i profili Adobe Color  predefiniti.
...
1. Aggiungete una proprietà di correzione colore scorrendo nella parte inferiore della scheda **[!UICONTROL Proprietà]** e immettendo manualmente il nome, il tipo e il valore della proprietà, descritti nelle tabelle seguenti. Dopo aver immesso i valori, toccare **[!UICONTROL Aggiungi]**, quindi **[!UICONTROL Salva tutto]** per salvare i valori.
   ...
   Ad esempio, in **[!UICONTROL Nome]**, aggiungere `iccprofilecmyk`, selezionare **[!UICONTROL Tipo]** `String` e aggiungere `WebCoated` come **[!UICONTROL Valore]**. Toccate **[!UICONTROL Aggiungi]**, quindi **[!UICONTROL Salva tutto]** per salvare i valori.
...
Ad esempio, è possibile impostare **[!UICONTROL iccprofilergb]** su `sRGB` e **[!UICONTROL iccprofilecmyk]** su `WebCoated`. In questo modo si effettua quanto segue:
...
Consultate [Distribuzione di risorse Dynamic Media](delivering-dynamic-media-assets.md).
