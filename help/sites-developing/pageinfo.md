...
Il servlet PageInfo restituisce informazioni sulle risorse nel repository. Il servlet è associato all’URL `https://<server>:<port>/libs/wcm/core/content/pageinfo.json` e utilizza il `path` parametro per identificare la risorsa. L&#39;URL di esempio seguente restituisce informazioni sul `/content/we-retail/us/en` nodo:
