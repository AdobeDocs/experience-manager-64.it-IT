...
1. Installa AEM.
1. Nel browser, immetti https://`<host>`:`<port>`/crx/de. Per impostazione predefinita, è `http://localhost:4502/crx/de`.
...
La funzione qui descritta è disponibile solo se il tipo di nodo `cq:Component` è disponibile nell’archivio.
...
1. In **Proprietà** nel riquadro inferiore, immetti la **Nome**, **Tipo** e **Valore**. Fai clic su **Aggiungi**.
...
Con CRXDE Lite è possibile importare e/o esportare le definizioni dei tipi di nodo in [Notazione CND (Compact Namespace and Node Type Definition)](http://jackrabbit.apache.org/jcr/node-type-notation.html).
