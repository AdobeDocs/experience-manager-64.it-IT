...
>[!NOTE]
>La funzione di editor di applicazioni a pagina singola (SPA) richiede [AEM 6.4 service pack 2](https://helpx.adobe.com/it/experience-manager/6-4/release-notes/sp-release-notes.html) o successivo.
>
...
Solo quando si risponde ad almeno una di queste due domande con un chiaro &quot;sì&quot; per il progetto, Adobe consiglia di implementare SSR. Nelle sezioni seguenti viene descritto come eseguire questa operazione utilizzando Adobe I/O Runtime.
...
AEM deve sapere dove è possibile recuperare il contenuto di cui è stato eseguito il rendering in remoto. Indipendentemente dal modello [che scegli di implementare per SSR](#adobe-io-runtime), dovrai specificare ad AEM come accedere a questo servizio di rendering remoto.
...
>[!NOTE]
>
>Indipendentemente dalla scelta di implementare il [flusso di comunicazione basato su AEM](#aem-driven-communication-flow) o il [flusso basato su Adobe I/O Runtime](#adobe-io-driven-communication-flow), è necessario definire una configurazione di rendering del contenuto remoto.
>
...
Quando utilizzi SSR, il [flusso di lavoro di interazione dei componenti](/help/sites-developing/spa-overview.md#workflow) delle applicazioni a pagina singola in AEM include una fase in cui il contenuto iniziale dell’app viene generato da Adobe I/O Runtime.
...
### Flusso di comunicazione basato su runtime di Adobe I/O {#adobe-io-driven-communication-flow}

La sezione [Flusso di comunicazione basato su AEM](#aem-driven-communication-flow) descrive l&#39;implementazione standard e consigliata del rendering lato server per quanto riguarda le applicazioni a pagina singola in AEM, dove AEM esegue il bootstrap e il serving dei contenuti.
...
Quando consideri l’implementazione del rendering lato server per la tua applicazione a pagina singola, devi verificare quali parti dell’app richiederà l’SSR.
