...
* **Configurazione Adobe Analytics :** Informazioni sul vostro account Adobe Analytics . La  configurazione Adobe Analytics consente AEM connettersi  Adobe Analytics. Create una configurazione Adobe Analytics  per ciascun account utilizzato.
...
>[!CAUTION]
>
>Fornire privilegi di **amministratore** (all&#39;interno  Adobe Analytics) non è sufficiente per consentire all&#39;utente di connettersi da AEM a  Adobe Analytics. L&#39;account deve inoltre disporre dei privilegi **Web Service Access** .
...
### Configurazione di AEM per l&#39;utilizzo dei centri dati Adobe Analytics  {#configuring-aem-to-use-your-adobe-analytics-data-centers}
 ...
 centri [](https://developer.omniture.com/en_US/content_page/concepts-terminology/c-how-is-data-stored) dati Adobe Analytics raccolgono, elaborano e memorizzano i dati associati alla suite di rapporti Adobe Analytics . Devi configurare AEM per utilizzare il centro dati che ospita la tua suite di rapporti Adobe Analytics . Nella tabella seguente sono elencati i centri dati disponibili e il relativo URL.
...
Utilizzate la console [Web per configurare il bundle](/help/sites-deploying/configuring-osgi.md#osgi-configuration-with-the-web-console) OSGi **Adobe AEM client** HTTP Analytics. Aggiungi l&#39;URL **del centro** dati per il centro dati che ospita una suite di rapporti per la quale le pagine AEM raccolgono i dati.

![a-07](assets/aa-07.png)

1. Aprite la console Web nel browser Web. ([http://localhost:4502/system/console/configMgr](http://localhost:4502/system/console/configMgr))
...
1. Selezionate l&#39;elemento Configuration denominato **Adobe AEM client** HTTP Analytics.
...
1. Utilizzando **Navigazione**, selezionare **Strumenti**, **Cloud Services**, quindi Cloud Services **** legacy.
...
6. Quindi utilizzate il menu a discesa **Modalità** esecuzione (accanto all&#39;ID della suite di rapporti) per selezionare le istanze del server che desiderate inviare informazioni alla suite di rapporti.
...
* **Namespace visitatore**
   ...
   * Ad esempio, modificando lo spazio dei nomi in **CNAME** , le chiamate effettuate a  Adobe Analytics avranno l’aspetto **CNAME.d1.omtrdc.net** invece del valore predefinito.
...
Per ulteriori informazioni, consulta [Visualizzazione dei dati](/help/sites-authoring/pa-using.md) di analisi delle pagine.
...
Come per la creazione di una configurazione o di un framework Adobe Analytics , passate alla schermata **Cloud Services** (legacy). Selezionate **Mostra configurazioni**, quindi fate clic sul collegamento alla configurazione specifica da aggiornare.
