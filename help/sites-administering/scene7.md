...
* Utilizzo di *Dynamic Media* integrato in Experience Manager.
...
Experience Manager Dynamic Media è l’unificazione delle funzioni di Dynamic Media Classic direttamente all’interno di Experience Manager Platform.
...
1. Segui le istruzioni specifiche relative al tuo caso d’uso:
   ...
   <table> 
    ...
    <td><p>Carica le risorse in Experience Manager e utilizza Dynamic Media per generare rappresentazioni da scaricare/condividere. Pubblicare automaticamente risorse di Experience Manager in Dynamic Media Classic per la distribuzione.</p> <p><strong>Importante:</strong> esegue l'elaborazione duplicata e le rappresentazioni generate in Experience Manager non vengono sincronizzate con Dynamic Media Classic</p> </td> 
    <td><p>Attivato</p> <p>(Vedere il passaggio 3)</p> </td> 
    <td>Disattivato</td> 
    <td>Disattivato</td> 
    <td><p><a href="#configuringautouploadingfromaemassets">Attivato</a></p> <p>(Vedere il passaggio 4)</p> </td> 
...
1. Aggiunta di componenti Dynamic Media Classic alla barra laterale. In questo modo gli utenti possono utilizzare i componenti di Dynamic Media Classic nelle loro pagine di Experience Manager.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>Le risorse che risiedono al di fuori della cartella di destinazione e caricate manualmente in Dynamic Media Classic in WCM vengono automaticamente posizionate in una cartella ad-hoc separata in Dynamic Media Classic. Puoi configurare questa funzionalità nella configurazione cloud in Experience Manager.</td> 
  </tr> 
  <tr> 
...
>[!NOTE]
>
>Puoi avere più configurazioni: una configurazione cloud rappresenta un utente in una società Dynamic Media Classic. Per accedere ad altre società o utenti di Dynamic Media Classic, crea più configurazioni.
...
A partire da Experience Manager 6.3.2, Risorse Experience Manager è ora configurato per te in modo che tutte le risorse digitali caricate nella gestione delle risorse digitali vengano aggiornate automaticamente in Dynamic Media Classic se le risorse si trovano in una cartella di destinazione CQ.
...
1. Segui le indicazioni per [configurare l&#39;anteprima protetta per il tuo account Dynamic Media Classic.](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish)
...
1. Tocca la scheda **[!UICONTROL Avanzate]** . Nel menu a discesa **[!UICONTROL Abilita visualizzazione protetta]** , seleziona **[!UICONTROL All’attivazione della pubblicazione AEM]** per inviare le risorse a Dynamic Media Classic senza la pubblicazione. (Per impostazione predefinita, questo valore è impostato su **[!UICONTROL Immediatamente]**, dove le risorse Dynamic Media Classic vengono pubblicate immediatamente.)
   ...
   Per ulteriori informazioni sui test delle risorse prima di renderle pubbliche, consulta [Documentazione di Dynamic Media Classic](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish) .
...
>[!CAUTION]
>
>L’importazione di risorse da un account aziendale Dynamic Media Classic esistente può richiedere molto tempo per essere visualizzata in Experience Manager. Assicurati di designare una cartella in Dynamic Media Classic priva di troppe risorse (ad esempio, la cartella principale spesso contiene troppe risorse).
>
