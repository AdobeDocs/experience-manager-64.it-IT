...
Per informazioni sull’utilizzo di tutti i componenti Dynamic Media Classic in una pagina e sull’utilizzo dei video, consulta [Utilizzo di Dynamic Media Classic](../assets/scene7.md).
...
>* Se utilizzi Dynamic Media Classic con una configurazione proxy personalizzata, configura entrambe le configurazioni proxy HTTP Client in quanto alcune funzionalità di Experience Manager utilizzano le API 3.x e altre le API 4.x. 3.x è configurato con [http://localhost:4502/system/console/configMgr/com.day.commons.httpclient](http://localhost:4502/system/console/configMgr/com.day.commons.httpclient) e 4.x è configurato con [http://localhost:4502/system/console/configMgr/org.apache.http.proxyconfigurator](http://localhost:4502/system/console/configMgr/org.apache.http.proxyconfigurator).
>


## Integrazione Experience Manager/Dynamic Media Classic con Dynamic Media {#aem-scene-integration-versus-dynamic-media}

Gli utenti di Experience Manager possono scegliere tra due soluzioni da utilizzare con Dynamic Media:

* Utilizzo dell’istanza integrata di Experience Manager con *Dynamic Media Classic*.
* Utilizzo *Dynamic Media* integrato nell&#39;Experience Manager.

Utilizza i seguenti criteri per determinare quale soluzione, se migliore per te:

* Se sei un **esistente** Cliente Dynamic Media Classic le cui risorse rich media risiedono in Dynamic Media Classic per la pubblicazione e la distribuzione, ma desideri integrarle con Sites (WCM) Authoring e/o Experience Manager Assets per la gestione, quindi utilizza il [Integrazione punto-punto Experience Manager/Dynamic Media Classic](#aem-scene-point-to-point-integration) descritto nel presente documento.
...
Experience Manager Dynamic Media è l’unificazione delle funzionalità di Dynamic Media Classic direttamente all’interno di Experience Manager Platform.
...
* **[!UICONTROL Dynamic Media]** - **[!UICONTROL Dynamic Media]** componente è intelligente : a seconda che aggiunga un’immagine o un video, avrai a disposizione diverse opzioni. Il componente supporta i predefiniti per immagini, i visualizzatori basati su immagini come set di immagini, set 360 gradi, set di file multimediali diversi e video. Inoltre, il visualizzatore è reattivo: le dimensioni dello schermo cambiano automaticamente in base alle dimensioni dello schermo. Tutti i visualizzatori sono visualizzatori HTML5.

* **[!UICONTROL File multimediali interattivi]** - **[!UICONTROL File multimediali interattivi]** Il componente è destinato ai banner a carosello, alle immagini interattive e ai video interattivi che hanno in essi interattività quali punti attivi o mappe immagine. Questo componente è intelligente. A seconda che si aggiunga un’immagine o un video, sono disponibili diverse opzioni. Inoltre, il visualizzatore è reattivo. In altre parole, le dimensioni dello schermo cambiano automaticamente in base alle dimensioni dello schermo. Tutti i visualizzatori sono visualizzatori HTML5.
...
1. Segui le istruzioni specifiche relative al tuo caso d’uso:
   ...
   <table> 
    ...
    <td><p>On</p> <p>(Vedere il passaggio 3)</p> </td> 
    ...
    <td>Off</td> 
    <td><a href="/help/assets/scene7.md#scene-content-browser">On</a></td> 
    <td><p><a href="#configuringautouploadingfromaemassets">On</a></p> <p>(Vedere il passaggio 4)</p> </td> 
    </tr> 
    <tr> 
    <td>Cliente Dynamic Media Classic esistente e nuovo di Assets</td> 
    <td><p>Carica le risorse in Experience Manager e utilizza Dynamic Media per generare rappresentazioni da scaricare/condividere. Pubblica automaticamente Experience Manager Assets in Dynamic Media Classic per la consegna.</p> <p><strong>Importante:</strong> Controlla l'elaborazione duplicata e le rappresentazioni generate in Experience Manager non vengono sincronizzate con Dynamic Media Classic</p> </td> 
    <td><p>On</p> <p>(Vedere il passaggio 3)</p> </td> 
    <td>Off</td> 
    <td>Off</td> 
    <td><p><a href="#configuringautouploadingfromaemassets">On</a></p> <p>(Vedere il passaggio 4)</p> </td> 
...
Se le risorse sono già in Dynamic Media Classic e la loro gestione in un archivio comune non è importante, esportarle in Experience Manager Assets solo per sincronizzarle nuovamente in Dynamic Media Classic per la consegna è un ciclo non necessario. In caso contrario, è preferibile mantenere le risorse in un unico archivio e sincronizzarle con Dynamic Media Classic solo per la distribuzione.
...
1. Se desideri che le risorse di Experience Manager vengano caricate automaticamente in Dynamic Media Classic, attiva l’opzione e aggiungi Dynamic Media Classic al flusso di lavoro delle risorse di aggiornamento DAM. Puoi anche caricare manualmente le risorse.
1. Aggiunta di componenti Dynamic Media Classic alla barra laterale. In questo modo gli utenti possono utilizzare i componenti Dynamic Media Classic nelle loro pagine di Experience Manager.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>Le risorse che risiedono al di fuori della cartella di destinazione e caricate manualmente in Dynamic Media Classic in WCM vengono automaticamente posizionate in una cartella ad-hoc separata su Dynamic Media Classic. Puoi configurare questa funzionalità nella configurazione cloud in Experience Manager.</td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><p>Se utilizzi le sottocartelle all’interno della cartella di destinazione, utilizza nomi univoci per ciascuna risorsa (indipendentemente dalla posizione) oppure configura Dynamic Media Classic (nell’area Configurazione) per non sovrascrivere le risorse indipendentemente dalla posizione.</p> <p>In caso contrario, vengono caricate le risorse con lo stesso nome e caricate in una sottocartella di destinazione di Dynamic Media Classic, ma la risorsa con lo stesso nome nella cartella di destinazione viene eliminata. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
>
>Se desideri testare l’integrazione dell’unità, fai in modo che la cartella principale punti solo a una sottocartella, invece che all’intera azienda.

>[!NOTE]
>
>Puoi avere più configurazioni: una configurazione cloud rappresenta un utente di una società Dynamic Media Classic. Per accedere ad altre aziende o utenti Dynamic Media Classic, crea più configurazioni.

Per configurare Experience Manager per la pubblicazione delle risorse in Dynamic Media Classic:

1. Tocca l’icona dell’Experience Manager e naviga per **[!UICONTROL Implementazione > Cloud Services]** per accedere a Adobe Dynamic Media Classic.

1. Tocca **[!UICONTROL Configura ora]**.

   ![chlimage_1-297](assets/chlimage_1-297.png)

1. In **[!UICONTROL Titolo]** ed eventualmente nel campo **[!UICONTROL Nome]** immettere le informazioni appropriate. Tocca **[!UICONTROL Crea]**.
...
1. Immetti l&#39;indirizzo e-mail, la password e la regione del tuo account Dynamic Media Classic e tocca **[!UICONTROL Connessione a Dynamic Media Classic]**. Sei connesso al server Dynamic Media Classic e la finestra di dialogo si espande con più opzioni.
...
1. Tocca [!UICONTROL Strumenti] , quindi passa a **[!UICONTROL Operazioni > Console web]**. Viene visualizzata la console Web.
1. Passa a **[!UICONTROL Listener Adobe CQ Dynamic Media Classic Dam]** e seleziona la **[!UICONTROL Abilitato]** casella di controllo.
...
1. Modifica il numero come desiderato nella **[!UICONTROL Timeout processo attivo]** campo . Qualsiasi numero non negativo è accettato con l&#39;unità di misura in secondi. Per impostazione predefinita, questo numero è impostato su 2100.
   ...
   >[!NOTE]
   >
   >Procedure consigliate: La maggior parte delle risorse vengono acquisite al massimo in pochi minuti (ad esempio, immagini). Tuttavia, in alcuni casi, ad esempio nei video più grandi, aumenta il valore di timeout a 7200 secondi (due ore) per adattarsi a tempi di elaborazione lunghi. In caso contrario, questo processo di caricamento di Dynamic Media Classic viene contrassegnato come **[!UICONTROL UploadFailed]** nei metadati JCR.

1. Tocca **[!UICONTROL Salva]**.

### Caricamento automatico da Experience Manager Assets {#autouploading-from-aem-assets}

A partire da Experience Manager 6.3.2, Experience Manager Assets è ora configurato per l’utente in modo che tutte le risorse digitali caricate nel gestore risorse digitali vengano aggiornate automaticamente in Dynamic Media Classic se le risorse si trovano in una cartella di destinazione CQ.
...
1. Tocca l’icona dell’Experience Manager e naviga per **[!UICONTROL Implementazione > Cloud Services]** quindi, sotto l’intestazione Dynamic Media, in Configurazioni disponibili, tocca **[!UICONTROL dms7 (Dynamic Media)]**)
1. Tocca **[!UICONTROL Avanzate]** seleziona la scheda **[!UICONTROL Abilita caricamento automatico]** casella di controllo, quindi toccare **[!UICONTROL OK]**. Ora devi configurare il flusso di lavoro DAM Asset per includere il caricamento in Dynamic Media Classic.
...
1. Torna alla pagina di benvenuto dell’Experience Manager e tocca **[!UICONTROL Flussi di lavoro]**. Fai doppio clic sul pulsante **Risorsa di aggiornamento DAM** flusso di lavoro per aprirlo.
1. Nella barra laterale passate alla **[!UICONTROL Flusso di lavoro]** componenti e seleziona **[!UICONTROL Dynamic Media Classic]**. Trascina **[!UICONTROL Dynamic Media Classic]** al flusso di lavoro e tocca **[!UICONTROL Salva]**. Le risorse aggiunte a Experience Manager Assets nella cartella di destinazione vengono caricate automaticamente in Dynamic Media Classic.
...
1. [Utilizza l’Admin Console per creare un caso di supporto.](https://helpx.adobe.com/it/enterprise/admin-guide.html/enterprise/using/support-for-experience-cloud.ug.html) Nel tuo caso di assistenza, richiedi che l’anteprima protetta sia abilitata per il tuo account Dynamic Media Classic.
1. Seguire le indicazioni per [imposta l&#39;anteprima protetta per il tuo account Dynamic Media Classic.](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish)
...
1. Tocca l’icona dell’Experience Manager e naviga per **[!UICONTROL Implementazione > Cloud Services]**, tocca **[!UICONTROL Dynamic Media Classic]**, quindi seleziona la configurazione in Dynamic Media Classic.
1. Tocca **[!UICONTROL Avanzate]** scheda . In **[!UICONTROL Abilita visualizzazione protetta]** menu a discesa, seleziona **[!UICONTROL All’attivazione di AEM Publish]** per inviare risorse in Dynamic Media Classic senza pubblicazione. Per impostazione predefinita, questo valore è impostato su **[!UICONTROL Immediatamente]**, in cui le risorse Dynamic Media Classic vengono pubblicate immediatamente.)

   Vedi [Documentazione di Dynamic Media Classic](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish) per ulteriori informazioni sulla verifica delle risorse prima di renderle pubbliche.
...
1. Tocca l’icona dell’Experience Manager e naviga per **[!UICONTROL Sites]** e quindi la pagina principale del sito web (non specifica della lingua).

1. Nella barra degli strumenti, seleziona la [!UICONTROL impostazioni] icona e tocco **[!UICONTROL Apri proprietà]**.

1. Tocca **[!UICONTROL Cloud Services]** e toccare **[!UICONTROL Aggiungi configurazione]** e seleziona **[!UICONTROL Dynamic Media Classic]**.
1. In **[!UICONTROL Adobe Dynamic Media Classic]** elenco a discesa, seleziona la configurazione desiderata e tocca **[!UICONTROL OK]**.
...
1. Ad Experience Manager, tocca **[!UICONTROL Siti Web]** e passare alla pagina principale del sito web (non specifica della lingua).

1. Nella barra laterale, tocca **[!UICONTROL Pagina]** icona e tocco **[!UICONTROL Proprietà pagina]**.
...
1. Tocca **[!UICONTROL Avanzate]** scheda . In **[!UICONTROL Cartella ad hoc]** puoi modificare il campo **Ad hoc** cartella. Per impostazione predefinita, è il **name_of_the_company/CQ5_adhoc**.

   ![chlimage_1-305](assets/chlimage_1-305.png)

### Configurazione dei predefiniti universali {#configuring-universal-presets}

Per configurare i predefiniti universali per il componente video, vedi [Video](/help/assets/s7-video.md).
...
1. Tocca l’icona dell’Experience Manager e naviga per **[!UICONTROL Strumenti > Operazioni > Console web]**.
1. Nel pannello Configurazione della console Web di Adobe Experience Manager, nella sezione **[!UICONTROL OSGi]** menu, tocca **[!UICONTROL Configurazione]**.
1. Nella colonna Nome , individua e tocca **[!UICONTROL Servizio Adobe CQ Dynamic Media Classic Asset MIME type]** per modificare la configurazione.
...
1. Sul lato destro della pagina CRXDE Lite, tocca **[!UICONTROL Proprietà]** scheda .
1. Specifica un parametro di processo di caricamento Dynamic Media Classic nel **[!UICONTROL jobParam]** campo valore.
   ...
   >
   >Assicurati che il tuo file PSD abbia &quot;livelli&quot;. Se è rigorosamente un&#39;immagine o un&#39;immagine con maschera, viene elaborata solo come immagine perché non ci sono livelli da elaborare.
...
* Verifica che la risorsa che stai tentando di caricare sia nella **[!UICONTROL Obiettivo CQ]** (specifica questa cartella nella configurazione cloud di Dynamic Media Classic).
* In caso contrario, devi configurare la configurazione cloud in **[!UICONTROL Proprietà pagina]** per consentire il caricamento in **[!UICONTROL Ad hoc CQ]** cartella.
...
* Se il caricamento del video non riesce e stai utilizzando un Experience Manager per codificare i video tramite l’integrazione Dynamic Media Classic, consulta [Aggiunta di un timeout configurabile al flusso di lavoro di caricamento Dynamic Media Classic](#adding-configurable-timeout-to-scene-upload-workflow).
